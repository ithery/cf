...
$.keyboard = function(el, options){
	...
	base.init = function(){
		...
		$(document).bind('mousedown.keyboard keyup.keyboard touchstart.keyboard', function(e){
			...
			if ( e.target && $(e.target).hasClass('vk-keyboard-input') ) {
		...
		base.$el
			.addClass('vk-keyboard-input ' + o.css.input);
			//.attr({ 'aria-haspopup' : 'true', 'role' : 'textbox' });

		// add disabled/readonly class - dynamically updated on reveal
		if (base.$el.is(':disabled') || (base.$el.attr('readonly') && !base.$el.hasClass('vk-keyboard-lockedinput'))) {
			base.$el.addClass('vk-keyboard-nokeyboard');
		}
		if (o.openOn) {
		...
		if (!base.watermark && base.$el.val() === '' && base.inPlaceholder !== '' && base.$el.attr('placeholder') !== '') {
			base.$el
				.addClass('vk-keyboard-placeholder') // css watermark style (darker text)
	...
	base.reveal = function(){
		base.opening = true;
		// close all keyboards
		$('.vk-keyboard:not(.vk-keyboard-always-open)').hide();
		...
		if (base.$el.is(':disabled') || (base.$el.attr('readonly') && !base.$el.hasClass('vk-keyboard-lockedinput'))) {
			...
			base.$el.removeClass('vk-keyboard-nokeyboard');
		}
		...
		// vk-keyboard-has-focus is applied in case multiple keyboards have alwaysOpen = true and are stacked
		$('.vk-keyboard-has-focus').removeClass('vk-keyboard-has-focus');
		$('.vk-keyboard-input-current').removeClass('vk-keyboard-input-current');

		base.$el.addClass('vk-keyboard-input-current');
		base.isCurrent(true);

		// clear watermark
		if (!base.watermark && base.el.value === base.inPlaceholder) {
			base.$el
				.removeClass('vk-keyboard-placeholder')
				.val('');
		}
		...
		// show keyboard
		base.$keyboard
			.addClass('vk-keyboard-has-focus')
			.show();
	...
	base.startup = function(){
		...
		base.$allKeys = base.$keyboard.find('button.vk-keyboard-button');
		base.preview = base.$preview[0];
		base.$decBtn = base.$keyboard.find('.vk-keyboard-dec');
		...
		base.$allKeys
			.bind(o.keyBinding.split(' ').join('.keyboard ') + '.keyboard repeater.keyboard', function(e){
				...
				if ($.keyboard.keyaction.hasOwnProperty(action) && $(this).hasClass('vk-keyboard-actionkey')) {
					// stop processing if action returns false (close & cancel)
					if ($.keyboard.keyaction[action](base,this,e) === false) { return; }
				} else if (typeof key.action !== 'undefined') {
					txt = base.lastKey = (base.wheel && !$(this).hasClass('vk-keyboard-actionkey')) ? key.curTxt : key.action;
					base.insertText(txt);
			...
			.filter(':not(.vk-keyboard-actionkey)')
			// mouse repeated action key exceptions
			.add('.vk-keyboard-tab, .vk-keyboard-bksp, .vk-keyboard-space, .vk-keyboard-enter', base.$keyboard)
	...
	base.showKeySet = function(el){
		...
		if (base.metaActive) {
			...
			if ( (!o.stickyShift && base.lastKeyset[2] !== base.metaActive) ||
				( (base.shiftActive || base.altActive) && !base.$keyboard.find('.vk-keyboard-keyset-' + key + base.rows[toShow]).length) ) {
		...
		if (!base.$keyboard.find('.vk-keyboard-keyset' + key + base.rows[toShow]).length) {
		...
		base.$keyboard
			.find('.vk-keyboard-alt, .vk-keyboard-shift, .vk-keyboard-actionkey[class*=meta]').removeClass(o.css.buttonAction).end()
			.find('.vk-keyboard-alt')[(base.altActive) ? 'addClass' : 'removeClass'](o.css.buttonAction).end()
			.find('.vk-keyboard-shift')[(base.shiftActive) ? 'addClass' : 'removeClass'](o.css.buttonAction).end()
			.find('.vk-keyboard-lock')[(base.capsLock) ? 'addClass' : 'removeClass'](o.css.buttonAction).end()
			.find('.vk-keyboard-keyset').hide().end()
			.find('.vk-keyboard-keyset' + key + base.rows[toShow]).show().end()
			.find('.vk-keyboard-actionkey.vk-keyboard' + key).addClass(o.css.buttonAction);
	...
	base.checkValid = function(){
		...
		base.$keyboard.find('.vk-keyboard-accept')
			[valid ? 'removeClass' : 'addClass']('vk-keyboard-invalid-input')
			[valid ? 'addClass' : 'removeClass']('vk-keyboard-valid-input');
	};
	...
	base.getLayers = function(el){
		...
		keys = el.closest('.vk-keyboard').find('button[data-pos="' + key + '"]').map(function(){
	...
	base.close = function(accepted){
		if (base.isVisible()) {
			...
			base.$el
				.removeClass('vk-keyboard-input-current vk-keyboard-autoaccepted')
				// add "vk-keyboard-autoaccepted" to inputs
				.addClass( (accepted || false) ? accepted === true ? '' : 'vk-keyboard-autoaccepted' : '' )
			...
			if (!base.watermark && base.el.value === '' && base.inPlaceholder !== '') {
				base.$el
					.addClass('vk-keyboard-placeholder')
					.val(base.inPlaceholder);
			}
		}
		return !!accepted;
	...
	base.addKey = function(keyName, name, regKey){
		...
		// Action keys will have the 'vk-keyboard-actionkey' class
		// '\u2190'.length = 1 because the unicode is converted, so if more than one character, add the wide class
		keyType = (n.length > 1) ? ' vk-keyboard-widekey' : '';
		keyType += (regKey) ? '' : ' vk-keyboard-actionkey';
		return base.keyBtn
			.clone()
			.attr({ 'data-value' : n, 'name': kn, 'data-pos': base.temp[1] + ',' + base.temp[2], 'title' : t })
			.data('key', { action: keyName, original: n, curTxt : n, curNum: 0 })
			// add "vk-keyboard-" + keyName, if this is an action key (e.g. "Bksp" will have 'vk-keyboard-bskp' class)
			// add "vk-keyboard-" + unicode of 1st character (e.g. "~" is a regular key, class = 'vk-keyboard-126' (126 is the unicode value - same as typing &#126;)
			.addClass('vk-keyboard-' + kn + keyType + ' ' + o.css.buttonDefault)
			//.html('<span>' + n + '</span>')
			.html('' + n + '')
	...
	base.buildKeyboard = function(){
		...
		container = $('<div />')
			.addClass('vk-keyboard ' + o.css.container + (o.alwaysOpen ? ' vk-keyboard-always-open' : '') )
			.attr({ 'role': 'textbox' })
			.hide();
		...
		if (o.usePreview) {
			base.$preview = base.$el.clone(false)
				.removeAttr('id')
				.removeClass('vk-keyboard-placeholder vk-keyboard-input')
				.addClass('vk-keyboard-preview ' + o.css.input)
				.attr('tabindex', '-1')
				.show(); // for hidden inputs
			// build preview container and append preview display
			$('<div />')
				.addClass('vk-keyboard-preview-wrapper')
				.append(base.$preview)
				.appendTo(container);
		} else {
		...
		if (o.lockInput) {
			base.$preview.addClass('vk-keyboard-lockedinput').attr({ 'readonly': 'readonly'});
		}
		...
		$.each($.keyboard.layouts[o.layout], function(set, keySet){
			if (set !== "") {
				...
				newSet = $('<div />')
					...
					.addClass('vk-keyboard-keyset vk-keyboard-keyset-' + set)
				...
				for ( row = 0; row < keySet.length; row++ ){
						...
						if( /^\{\S+\}$/.test(keys[key])){
							...
							if (/^sp:((\d+)?([\.|,]\d+)?)(em|px)?$/.test(action)) {
								...
								$('<span>&nbsp;</span>')
									...
									.addClass('vk-keyboard-button vk-keyboard-spacer')
									.appendTo(newSet);
							}
					...
					newSet.find('.vk-keyboard-button:last').after('<br class="vk-keyboard-button-endrow">');
				}
			}
		});
	...
	base.destroy = function() {
		...
		base.$el
			.removeClass('vk-keyboard-input vk-keyboard-lockedinput vk-keyboard-placeholder vk-keyboard-notallowed vk-keyboard-always-open ' + o.css.input)
	...
	$.keyboard.keyaction = {
		...
		combo : function(base){
			...
			base.$keyboard.find('.vk-keyboard-combo')[(c) ? 'addClass' : 'removeClass'](base.options.css.buttonAction);
	...
	$.keyboard.defaultOptions = {
		...
		*/
		...
		css : {
			input          : 'vk-input', // input & preview
			container      : 'modal ', // keyboard container
			buttonDefault  : 'vk-state-default vk-corner-all', // default state
			buttonHover    : 'vk-state-hover',  // hovered button
			buttonAction   : 'vk-state-active', // Action keys (e.g. Accept, Cancel, Tab, etc); this replaces "actionClass" option
			buttonDisabled : 'vk-state-disabled' // used when disabling the decimal button {dec} when a decimal exists in the input area
		},
		...
		// Check input against validate function, if valid the accept button gets a class name of "vk-keyboard-valid-input"
		// if invalid, the accept button gets a class name of "vk-keyboard-invalid-input"
		acceptValid  : false,
