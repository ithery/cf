export function cresDirectives(el) {
...
class DirectiveManager {
    ...
    extractTypeModifiersAndValue() {
        return Array.from(this.el.getAttributeNames()
            // Filter only the cresenity directives.
            .filter(name => name.match(new RegExp('cres:')))
            // Parse out the type, modifiers, and value from it.
            .map(name => {
                const [type, ...modifiers] = name.replace(new RegExp('cres:'), '').split('.');
