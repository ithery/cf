...
export default function() {
    ...
    window.addEventListener('popstate', event => {
        if (CresenityStateManager.missingState(event)) return

        CresenityStateManager.replayResponses(event, (response, component) => {
    ...
    function normalizeResponse(response, component) {
        ...
        // Sometimes Cresenity doesn't return html from the server to save on bandwidth.
    ...
    store.registerHook('element.updating', (from, to, component) => {
        ...
        if (from.getAttribute('cf:id') === component.id) {
    ...
    store.registerHook('element.updated', (node, component) => {
        ...
        if (component.lastKnownDomId) {
            // Let's check and see if the cf:id was the thing that changed.
            if (node.getAttribute('cf:id') !== component.lastKnownDomId) {
                // If so, we need to change this ID globally everwhere it's referenced.
                store.changeComponentId(component, node.getAttribute('cf:id'))
            }
...
let CresenityStateManager = {
    ...
    replayResponses(event, callback) {
        if (!event.state.cresenity) return

        let state = typeof event.state.cresenity === 'string' ?
            new CresenityState(this.getFromSession(event.state.cresenity)) :
            new CresenityState(event.state.cresenity)

        state.replayResponses(callback)
    },

    currentState() {
        if (!history.state) return new CresenityState
        if (!history.state.cresenity) return new CresenityState

        let state = typeof history.state.cresenity === 'string' ?
            new CresenityState(this.getFromSession(history.state.cresenity)) :
            new CresenityState(history.state.cresenity)
    ...
    storeInSession(value) {
        let key = 'cresenity:' + (new Date).getTime()
    ...
    tryToStoreInSession(key, value) {
        ...
        try {
            ...
            let oldestTimestamp = Object.keys(sessionStorage)
                .map(key => Number(key.replace('cresenity:', '')))
                .sort()
                .shift()
...
class CresenityState {
    ...
    findComponentBySignature(signature) {
        ...
        return sameNamedComponents[componentIndex] || sameNamedComponents[0] || console.warn(`Cresenity: couldn't find component on page: ${componentName}`)
    }
