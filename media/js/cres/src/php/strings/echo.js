class echo {
    constructor() {
        ...
        //   returns 1: undefined
        let isNode = typeof module !== 'undefined' && module.exports && typeof global !== 'undefined' && {}.toString.call(
        ...
        let stringToDOM = function (str, parent, ns, container) {
            let extraNSs = '';
            if (ns === ns_xul) {
                extraNSs = ' xmlns:html="' + ns_xhtml + '"';
            }
            let stringContainer = '<' + container + ' xmlns="' + ns + '"' + extraNSs + '>' + str + '</' + container + '>';
            let dils = win.DOMImplementationLS;
            let dp = win.DOMParser;
            let ax = win.ActiveXObject;
            if (dils && dils.createLSInput && dils.createLSParser) {
                // Follows the DOM 3 Load and Save standard, but not
                // implemented in browsers at present; HTML5 is to standardize on innerHTML, but not for XML (though
                // possibly will also standardize with DOMParser); in the meantime, to ensure fullest browser support, could
                // attach http://svn2.assembla.com/svn/brettz9/DOMToString/DOM3.js (see http://svn2.assembla.com/svn/brettz9/DOMToString/DOM3.xhtml for a simple test file)
                let lsInput = dils.createLSInput();
                // If we're in XHTML, we'll try to allow the XHTML namespace to be available by default
                lsInput.stringData = stringContainer;
                // synchronous, no schema type
                let lsParser = dils.createLSParser(1, null);
                ...
                let axo = new ax('MSXML2.DOMDocument');
                axo.loadXML(str);
        ...
        let ieFix = function (node) {
            if (node.nodeType === 1) {
                let newNode = d.createElement(node.nodeName);
                let i, len;
            ...
            if (d.appendChild) {
                if (d.body) {
                    if (win.navigator.appName === 'Microsoft Internet Explorer') {
                        ...
                        let unappendedLeft = stringToDOM(arg, d.body, ns_xhtml, 'div')
