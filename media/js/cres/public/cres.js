...
(function (factory) {
    ...
    _createClass(CF, [{
      ...
      value: function loadJQuery(callback) {
        ...
        if (typeof jQuery == 'undefined') {
          ...
          fileref.setAttribute('src', this.getConfig().defaultJQueryUrl); // IE 6 & 7
        ...
        this.loadJQuery(function () {
          if (typeof arrayJsUrl !== 'undefined') {
            arrayJsUrl.forEach(function (item) {
              _this4.required.push(item);
            });
          }

          if (typeof arrayCssUrl !== 'undefined') {
            arrayCssUrl.forEach(function (item) {
              _this4.cssRequired.push(item);
            });
          }

          _this4.afterInitCallback.forEach(function (item) {
            item();
          });
        });
      }
  ...
  function isValidObject(val) {
    return isObject$a(val) || Array.isArray(val) || typeof val === 'function';
  }
  ...
  var DOM = {
    ...
    setInputValue: function setInputValue(el, value) {
      store$5.callHook('interceptWireModelSetValue', value, el);
  ...
  var store$4 = global$l[SHARED] || setGlobal$2(SHARED, {});
  ...
  */
  ...
  var Connection = /*#__PURE__*/function () {
    ...
    _createClass(Connection, [{
      ...
      value: function onError(message, status) {
        ...
        return store$5.onErrorCallback(status);
      }
    }, {
      key: "sendMessage",
        ...
        fetch("".concat(window.capp.baseUrl, "cresenity/component/message/").concat(payload.fingerprint.name), {
          ...
          if (response.ok) {
            ...
            if (response.status === 419) {
              if (store$5.sessionHasExpired) {
                return;
              }

              store$5.sessionHasExpired = true; // eslint-disable-next-line no-alert
  ...
  function Polling () {
    store$5.registerHook('element.initialized', function (el, component) {
    ...
    store$5.registerHook('element.updating', function (from, to, component) {
  ...
  function fireActionOnInterval(node, component) {
    ...
    return setInterval(function () {
      ...
      if (store$5.cresenityIsInBackground && !directive.modifiers.includes('keep-alive')) {
  ...
  var nodeInitializer = {
    initialize: function initialize(el, component) {
      ...
      if (store$5.initialRenderIsFinished && el.tagName.toLowerCase() === 'script') {
        eval(el.innerHTML);
        return false;
      }
      ...
      cresDirectives(el).all().forEach(function (directive) {
        switch (directive.type) {
          ...
          default:
            if (store$5.directives.has(directive.type)) {
              store$5.directives.call(directive.type, el, directive, component);
            }
      ...
      store$5.callHook('element.initialized', el, component);
    },
    ...
    attachModelListener: function attachModelListener(el, directive, component) {
      ...
      store$5.callHook('interceptWireModelAttachListener', directive, el, component); // File uploads are handled by UploadFiles.js.
      ...
      var handler = function handler(e) {
        ...
        component.callAfterModelDebounce(function () {
          ...
          if (method === '$emit') {
            ...
            store$5.emit.apply(store$5, _toConsumableArray(params));
            return;
          }

          if (method === '$emitUp') {
            store$5.emitUp.apply(store$5, [el].concat(_toConsumableArray(params)));
            return;
          }

          if (method === '$emitSelf') {
            store$5.emitSelf.apply(store$5, [component.id].concat(_toConsumableArray(params)));
            return;
          }

          if (method === '$emitTo') {
            store$5.emitTo.apply(store$5, _toConsumableArray(params));
            return;
          }
  ...
  function LoadingStates () {
    store$5.registerHook('component.initialized', function (component) {
    ...
    store$5.registerHook('message.sent', function (message, component) {
    ...
    store$5.registerHook('message.failed', function (message, component) {
      unsetLoading(component);
    });
    store$5.registerHook('message.received', function (message, component) {
      unsetLoading(component);
    });
    store$5.registerHook('element.removed', function (el, component) {
  ...
  function refreshAlpineAfterEveryCresenityRequest() {
    if (isV3()) {
      store$5.registerHook('message.processed', function (message, cresenityComponent) {
  ...
  var Component = /*#__PURE__*/function () {
    function Component(el, connection) {
      ...
      this.watchers = {};
      store$5.callHook('component.initialized', this);
      this.initialize();
    ...
    _createClass(Component, [{
      ...
      value: function initialize() {
        ...
        function (el) {
          return store$5.addComponent(new Component(el, _this.connection));
        });
      }
    }, {
      key: "get",
      ...
      value: function fireMessage() {
        ...
        var sendMessage = function sendMessage() {
          ...
          store$5.callHook('message.sent', _this4.messageInTransit, _this4);
          _this4.updateQueue = [];
        };
      ...
      value: function messageSendFailed() {
        store$5.callHook('message.failed', this.messageInTransit, this);
      ...
      value: function handleResponse(message) {
        ...
        store$5.callHook('message.received', message, this);
        ...
        if (!message.replaying) {
          ...
          if (response.effects.emits && response.effects.emits.length > 0) {
            response.effects.emits.forEach(function (event) {
              ...
              if (event.selfOnly) {
                store$5.emitSelf.apply(store$5, [_this5.id, event.event].concat(_toConsumableArray(event.params)));
              } else if (event.to) {
                store$5.emitTo.apply(store$5, [event.to, event.event].concat(_toConsumableArray(event.params)));
              } else if (event.ancestorsOnly) {
                store$5.emitUp.apply(store$5, [_this5.el, event.event].concat(_toConsumableArray(event.params)));
              } else {
                store$5.emit.apply(store$5, [event.event].concat(_toConsumableArray(event.params)));
              }
            });
          }
        ...
        store$5.callHook('message.processed', message, this);
      }
    }, {
      key: "redirect",
      ...
      value: function handleMorph(dom) {
        ...
        morphdom(this.el, dom, {
          ...
          onNodeDiscarded: function onNodeDiscarded(node) {
            store$5.callHook('element.removed', node, _this7);

            if (node.__cresenity) {
              store$5.removeComponent(node.__cresenity);
            }
          ...
          onBeforeElUpdated: function onBeforeElUpdated(from, to) {
            ...
            store$5.callHook('element.updating', from, to, _this7); // Reset the index of cres:modeled select elements in the
          ...
          onElUpdated: function onElUpdated(node) {
            ...
            store$5.callHook('element.updated', node, _this7);
          },
          onNodeAdded: function onNodeAdded(node) {
            ...
            if (closestComponentId === _this7.id) {
              ...
              store$5.addComponent(new Component(node, _this7.connection)); // We don't need to initialize children, the
            ...
            if (typeof property === 'string' && property.match(/^emit.*/)) {
              return function () {
                ...
                if (property === 'emitSelf') {
                  return store$5.emitSelf.apply(store$5, [component.id].concat(args));
                }

                return store$5[property].apply(component, args);
              };
            }
  ...
  function FileUploads () {
    ...
    store$5.registerHook('interceptWireModelAttachListener', function (directive, el, component) {
      if (!(el.tagName.toLowerCase() === 'input' && el.type === 'file')) {
        return;
      }
          ...
          if (event.startsWith('echo')) {
            ...
            if (['channel', 'private'].includes(channel_type)) {
              ...
              Echo.private(channel).notification(function (notification) {
                store$5.emit(event, notification);
              });
            } else {
  ...
  function OfflineStates () {
    store$5.registerHook('element.initialized', function (el) {
    ...
    window.addEventListener('offline', function () {
      store$5.cresenityIsOffline = true;
      offlineEls.forEach(function (el) {
        toggleOffline(el, true);
      });
    });
    window.addEventListener('online', function () {
      store$5.cresenityIsOffline = false;
      offlineEls.forEach(function (el) {
        toggleOffline(el, false);
      });
    });
    store$5.registerHook('element.removed', function (el) {
  ...
  function SyncBrowserHistory () {
    ...
    store$5.registerHook('message.processed', function (message, component) {
    ...
    store$5.registerHook('element.updated', function (node, component) {
      ...
      if (component.lastKnownDomId) {
        ...
        if (node.getAttribute('cres:id') !== component.lastKnownDomId) {
          ...
          store$5.changeComponentId(component, node.getAttribute('cres:id'));
  ...
  var CresenityStateManager = {
    ...
    updateState: function updateState(method, url, response, component) {
      ...
      store$5.callHook('before' + capitalize(method), fullstateObject, url, component);
  ...
  var CresenityState = /*#__PURE__*/function () {
    ...
    _createClass(CresenityState, [{
      ...
      value: function pushItemInProperOrder(signature, response, component) {
        ...
        var closestParentId = store$5.getClosestParentId(component.id, this.componentIdsWithStoredResponses());
      ...
      value: function getComponentNameBasedSignature(component) {
        ...
        var sameNamedComponents = store$5.getComponentsByName(componentName);
      ...
      value: function findComponentBySignature(signature) {
        ...
        var sameNamedComponents = store$5.getComponentsByName(componentName); // If we found the component in the proper place, return it,
