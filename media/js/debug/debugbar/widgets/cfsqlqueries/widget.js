(function ($) {
     ...
     *  - data
     */
    var CFSQLQueriesWidget = (PhpDebugBar.Widgets.CFSQLQueriesWidget =
            ...
            render: function () {
                ...
                this.$list = new PhpDebugBar.Widgets.ListWidget({
                    itemRenderer: function (li, stmt) {
                        ...
                        if (
                            typeof stmt.is_success != 'undefined' &&
                            !stmt.is_success
                        ) {
                        ...
                        if (stmt.bindings && stmt.bindings.length) {
                            table.append(function () {
                                ...
                                var index = 0;
                                var $bindings =
                                    new PhpDebugBar.Widgets.ListWidget({
                ...
                this.bindAttr('data', function (data) {
                        ...
                        if (sql[stmt].keys.length > 1) {
                            ...
                            for (i = 0; i < sql[stmt].keys.length; i++) {
                                this.$list.$el
                                    ...
                                    .addClass(
                                        csscls('sql-duplicate-' + duplicate)
                                    );
                            }
                        }
                    }
                    ...
                    if (data.nb_failed_statements) {
                        t.append(
                            ', ' +
                                data.nb_failed_statements +
                                ' of which failed'
                        );
                    }
                    if (duplicate) {
                        t.append(
                            ', ' + duplicate + ' of which were duplicated'
                        );
                        t.append(
                            ', ' + (data.nb_statements - duplicate) + ' unique'
                        );
                    }
