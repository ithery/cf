(function ($) {
     ...
     *  - data
     */
    window.CFSQLQueriesWidget = (window.PhpDebugBar.Widgets.CFSQLQueriesWidget =
        window.PhpDebugBar.Widget.extend({
            ...
            render: function () {
                ...
                this.$list = new window.PhpDebugBar.Widgets.ListWidget({
                    itemRenderer: function (li, stmt) {
                        if (stmt.type === 'transaction') {
                            $('<strong />').addClass(csscls('sql')).addClass(csscls('name')).text(stmt.sql).appendTo(li);
                        } else {
                            $('<code />').addClass(csscls('sql')).html(window.PhpDebugBar.Widgets.highlight(stmt.sql, 'sql')).appendTo(li);
                        }
                        ...
                        if (stmt.bindings && stmt.bindings.length) {
                            table.append(function () {
                                ...
                                var index = 0;
                                var $bindings = new window.PhpDebugBar.Widgets.ListWidget({ itemRenderer: function (li, binding) {
                        ...
                        if (stmt.hints && stmt.hints.length) {
                            table.append(function () {
                                ...
                                var $hints = new window.PhpDebugBar.Widgets.ListWidget({ itemRenderer: function (li, hint) {
                        ...
                        if (stmt.backtrace && stmt.backtrace.length) {
                            table.append(function () {
                                ...
                                var $backtrace = new window.PhpDebugBar.Widgets.ListWidget({ itemRenderer: function (li, source) {
                                    var $parts = [
