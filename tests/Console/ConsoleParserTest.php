...
class ConsoleParserTest extends TestCase {
    public function testBasicParameterParsing() {
        $results = CConsole_Parser::parse('command:name');

        $this->assertSame('command:name', $results[0]);

        $results = CConsole_Parser::parse('command:name {argument} {--option}');
        ...
        $results = CConsole_Parser::parse('command:name {argument?* : The argument description.}    {--option=* : The option description.}');
    ...
    public function testShortcutNameParsing() {
        $results = CConsole_Parser::parse('command:name {--o|option}');
        ...
        $results = CConsole_Parser::parse('command:name {--o|option=* : The option description.}');
    ...
    public function testDefaultValueParsing() {
        $results = CConsole_Parser::parse('command:name {argument=defaultArgumentValue} {--option=defaultOptionValue}');
        ...
        $results = CConsole_Parser::parse('command:name {argument=*defaultArgumentValue1,defaultArgumentValue2} {--option=*defaultOptionValue1,defaultOptionValue2}');
    ...
    public function testArgumentDefaultValue() {
        $results = CConsole_Parser::parse('command:name {argument= : The argument description.}');
        $this->assertNull($results[1][0]->getDefault());

        $results = CConsole_Parser::parse('command:name {argument=default : The argument description.}');
        $this->assertSame('default', $results[1][0]->getDefault());
    }

    public function testOptionDefaultValue() {
        $results = CConsole_Parser::parse('command:name {--option= : The option description.}');
        $this->assertNull($results[2][0]->getDefault());

        $results = CConsole_Parser::parse('command:name {--option=default : The option description.}');
