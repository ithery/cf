...
final class Builder
{
    ...
    private function addItems(Directory $root, array $items, array $tests)
    {
     ...
     * </code>
     */
    private function buildDirectoryStructure(ProcessedCodeCoverageData $data)
    {
        $result = [];
        ...
        foreach ($data->coveredFiles() as $originalPath) {
            ...
            $dataLineCoverage = $data->lineCoverage();
            $dataFunctionCoverage = $data->functionCoverage();

            $pointer = [
                'lineCoverage'     => isset($dataLineCoverage[$originalPath]) ? $dataLineCoverage[$originalPath] : [],
                'functionCoverage' => isset($dataFunctionCoverage[$originalPath]) ? $dataFunctionCoverage[$originalPath] : [],
            ];
        }
     ...
     * </code>
     */
    private function reducePaths(ProcessedCodeCoverageData $coverage)
    {
