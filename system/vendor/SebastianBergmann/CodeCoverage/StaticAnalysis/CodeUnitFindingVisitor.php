...
final class CodeUnitFindingVisitor extends NodeVisitorAbstract
{
     ...
     */
    private function cyclomaticComplexity(Node $node)
    {
     ...
     * @psalm-param ClassMethod|Function_ $node
     */
    private function signature(Node $node)
    {
        assert($node instanceof ClassMethod || $node instanceof Function_);
     ...
     */
    private function type(Node $type)
    {
    ...
    private function visibility(ClassMethod $node)
    {
        if ($node->isPrivate()) {
            return 'private';
        }
    ...
    private function processClass(Class_ $node)
    {
        ...
        $this->classes[$namespacedName] = [
            ...
            'namespace'      => $this->getNamespace($namespacedName, $name),
    ...
    private function processTrait(Trait_ $node)
    {
        $name           = $node->name->toString();
        $namespacedName = $node->namespacedName->toString();
    ...
    private function processMethod(ClassMethod $node)
    {
        $parentNode = $node->getAttribute('parent');
        ...
        if (!isset($storage[$parentNamespacedName])) {
            $storage[$parentNamespacedName] = [
                ...
                'namespace'      => $this->getNamespace($parentNamespacedName, $parentName),
    ...
    private function processFunction(Function_ $node)
    {
    ...
    private function getNamespace($namespacedName, $name)
    {
