...
class DateTimeComparator extends ObjectComparator
{
    ...
    public function assertEquals($expected, $actual, $delta = 0.0, $canonicalize = false, $ignoreCase = false, array &$processed = [])/*: void*/
    {
        ...
        $tempCloneActual = (clone $actual);
        $actualClone = $tempCloneActual
                ->setTimezone(new DateTimeZone('UTC'));

        $tempCloneExpectedLower = (clone $expected);
        $expectedLower = $tempCloneExpectedLower
            ->setTimezone(new DateTimeZone('UTC'))
            ->sub($delta);
        
        $tempCloneExpectedUpper = (clone $expected);
        $expectedUpper = $tempCloneExpectedUpper
            ->setTimezone(new DateTimeZone('UTC'))
            ->add($delta);
