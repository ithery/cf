...
class OptionsResolver implements Options {
    ...
    public function remove($optionNames) {
        ...
        foreach ((array) $optionNames as $option) {
            unset($this->defined[$option], $this->defaults[$option], $this->required[$option], $this->resolved[$option], $this->lazy[$option], $this->normalizers[$option], $this->allowedTypes[$option], $this->allowedValues[$option], $this->info[$option]);
        }
                ...
                $allowedContainsArrayType = \count(array_filter($this->allowedTypes[$option], static function ($item) {
                    ...
                    foreach ((array) $needles as $needle) {
                        if ($needle !== '' && $needle !== null
                            && substr($item, -strlen($needle)) === (string) $needle
                        ) {
                            return true;
                        }
                    }
    ...
    private function verifyTypes(string $type, $value, array &$invalidTypes, int $level = 0): bool {
        ...
        if (!$invalidTypes || $level > 0) {
            ...
            $invalidTypes[gettype($value)] = true;
        }
