...
class HtmlErrorRenderer implements ErrorRendererInterface {
    ...
    public function __construct($debug = false, $charset = null, $fileLinkFormat = null, $projectDir = null, $outputBuffer = '', LoggerInterface $logger = null) {
        if (!\is_bool($debug) && !\is_callable($debug)) {
            throw new \TypeError(sprintf('Argument 1 passed to "%s()" must be a boolean or a callable, "%s" given.', __METHOD__, function_exists('get_debug_type') ? get_debug_type($debug) : gettype($debug)));
        }

        if (!\is_string($outputBuffer) && !\is_callable($outputBuffer)) {
            throw new \TypeError(sprintf('Argument 5 passed to "%s()" must be a string or a callable, "%s" given.', __METHOD__, function_exists('get_debug_type') ? get_debug_type($outputBuffer) : gettype($outputBuffer)));
        }
    ...
    public function getStylesheet() {
        if (!$this->debug) {
            return $this->getInclude('assets/css/error.css');
        }

        return $this->getInclude('assets/css/exception.css');
    }

    public static function isDebug(RequestStack $requestStack, $debug) {
        return static function () use ($requestStack, $debug) {
    ...
    private function renderException(FlattenException $exception, $debugTemplate = 'views/exception_full.html.php') {
        ...
        if (!$debug) {
            return $this->getInclude(self::$template, [
        ...
        if (!is_string($outputBuffer)) {
            $outputBuffer = $outputBuffer();
        }

        return $this->getInclude($debugTemplate, [
                    'exception' => $exception,
                    'exceptionMessage' => $exceptionMessage,
                    'statusText' => $statusText,
                    'statusCode' => $statusCode,
                    'logger' => $this->logger instanceof DebugLoggerInterface ? $this->logger : null,
                    'currentContent' => $outputBuffer,
        ]);
    }
    ...
    private function getInclude($name, array $context = []) {
        ...
        $filename = file_exists($name) ? $name : __DIR__ . '/../Resources/' . $name;
        include $filename;
