...
class Client extends AbstractTus {
    ...
    public function createWithUpload(string $key, int $bytes = -1): array {
        ...
        try {
            ...
            \cdbg::dd('error', $e->getMessage(), $e->getTraceAsString());
        }

        $statusCode = $response->getStatusCode();
        // \cdbg::dd($this->apiPath, $response->getBody()->getContents());
    ...
    protected function sendPatchRequest(int $bytes, int $offset): int {
        ...
        try {
            ...
            throw new Exception('Error Bad Response:' . $e->getResponse()->getBody()->getContents());
        } catch (ConnectException $e) {
            throw new ConnectionException("Couldn't connect to server.");
        }
    }
