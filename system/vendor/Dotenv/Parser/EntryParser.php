...
final class EntryParser
{
    ...
    public static function parse( $entry)
    {
        return self::splitStringIntoParts($entry)->flatMap(static function (array $parts) {
            ...
            return self::parseName($name)->flatMap(static function ( $name) use ($value) {
                /** @var Result<Value|null,string> */
                $parsedValue = $value === null ? Success::create(null) : self::parseValue($value);

                return $parsedValue->map(static function ( $value) use ($name) {
     ...
     * @return bool
     */
    private static function isQuotedName( $name)
    {
        if (Str::len($name) < 3) {
            return false;
        }
    ...
    private static function parseValue( $value)
    {
        ...
        return \array_reduce(\iterator_to_array(Lexer::lex($value)), static function (Result $data,  $token) use ($value) {
            return $data->flatMap(static function (array $data) use ($token, $value) {
                return self::processToken($data[1], $token)->mapError(static function ( $err) use ($value) {
     ...
     */
    private static function processToken( $state,  $token)
    {
        switch ($state) {
