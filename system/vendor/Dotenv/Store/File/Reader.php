...
final class Reader
{
     ...
     */
    public static function read(array $filePaths,  $shortCircuit = true,  $fileEncoding = null)
    {
        $output = [];
     ...
     */
    private static function readFromFile( $path,  $encoding = null)
    {
        /** @var Option<string> */
        $content = Option::fromValue(@\file_get_contents($path), false);

        return $content->flatMap(static function ( $content) use ($encoding) {
            return Str::utf8($content, $encoding)->mapError(static function ( $error) {
