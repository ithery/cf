...
final class ReflectionClassDefinitionRepository implements ClassDefinitionRepository
{
    ...
    private function methods(ObjectType $type): array
    {
        ...
        $methods = array_filter(
            ...
            array($this, 'shouldMethodBeIncluded') // versi PHP 7
        );
    ...
    private function shouldMethodBeIncluded(ReflectionMethod $method): bool
    {
        if (
         ...
         // Backward compat getAttributes()
        if (method_exists($method, 'getAttributes')) {
            return $method->getAttributes(Constructor::class) !== [];
        }

        // PHP 7.4 fallback: cek lewat docblock atau cara lain
        $docComment = $method->getDocComment();
        if ($docComment && stripos($docComment, '@' . (new \ReflectionClass(Constructor::class))->getShortName()) !== false) {
            return true;
        }
