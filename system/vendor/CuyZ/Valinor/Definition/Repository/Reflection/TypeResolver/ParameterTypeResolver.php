...
{
    private ReflectionTypeResolver $typeResolver;
    public function __construct(ReflectionTypeResolver $typeResolver) {
    ...
    public function resolveTypeFor(ReflectionParameter $reflection): Type
    {
        ...
        if (method_exists($reflection, 'isPromoted') && $reflection->isPromoted()) {
            ...
            $property = $reflection->getDeclaringClass()->getProperty($reflection->name);
            $docBlockType = (new PropertyTypeResolver($this->typeResolver))
                ->extractTypeFromDocBlock($property);
        }
        ...
        // $type = $this->typeResolver->resolveType($reflection->getType(), $docBlockType);
         ...
         // Backward compat getType() untuk PHP 7.4
        $refType = method_exists($reflection, 'getType') ? $reflection->getType() : null;
        $type = $this->typeResolver->resolveType($refType, $docBlockType);
