...
final class ValueConverterNodeBuilder implements NodeBuilder
{
    ...
    public function build(Shell $shell, RootNodeBuilder $rootBuilder): Node
    {
        ...
        $converterAttributes = $attributes->filter(
            array($this->converterContainer, 'filterConverterAttributes')
        );

        $stack = array_merge(
            array_map(
                // @phpstan-ignore method.notFound (we know the `map` method exists)
                static function (AttributeDefinition $attribute) {
                    return $attribute->instantiate()->map();
                },
                $converterAttributes->toArray()
            ),
            $this->converterContainer->converters()
     ...
     * @return mixed
     */
    private function unstack(array $stack, Shell $shell, RootNodeBuilder $rootBuilder)
    {
        while ($current = array_shift($stack)) {
            ...
            // if($converter->parameters->at(0)->type instanceof \CuyZ\Valinor\Type\Types\UnresolvableType) {
            //     \cdbg::dd($converter->parameters->at(0));
            // }
            ...
            if ($converter->parameters->count() > 1) {
                $arguments[] = function ($value = NAN) use ($stack, $shell, $rootBuilder) {
