...
final class GuzzleHandler {
    ...
    private function createApiRequest(SignIn $action): RequestInterface {
        if ($action instanceof SignInAnonymously) {
            return $this->anonymous($action);
        }

        if ($action instanceof SignInWithCustomToken) {
            return $this->customToken($action);
        }

        if ($action instanceof SignInWithEmailAndPassword) {
            return $this->emailAndPassword($action);
        }

        if ($action instanceof SignInWithEmailAndOobCode) {
            return $this->emailAndOobCode($action);
        }

        if ($action instanceof SignInWithIdpCredentials) {
            return $this->idpCredentials($action);
        }

        if ($action instanceof SignInWithRefreshToken) {
