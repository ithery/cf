 ...
 */
class CDebug_Storage_FilesystemStorage implements CDebug_Contract_StorageInterface {
    protected $dirname;
    ...
    public function save($id, $data) {
        if (!CFile::isDirectory($this->dirname)) {
            if (CFile::makeDirectory($this->dirname, 0777, true)) {
                CFile::put($this->dirname . '.gitignore', "*\n!.gitignore\n");
            } else {
        ...
        try {
            CFile::put($this->makeFilename($id), json_encode($data));
        } catch (\Exception $e) {
            //TODO; error handling
        }
    ...
    public function get($id) {
        return json_decode(CFile::get($this->makeFilename($id)), true);
    }
