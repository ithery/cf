...
class CDebug_DebugBar_DataCollector_ViewCollector extends TwigCollector {
    protected $name;
    ...
    public function getWidgets() {
        return [
            'views' => [
                'icon' => 'leaf',
                'widget' => 'PhpDebugBar.Widgets.CFViewWidget',
                'map' => 'views',
                'default' => '[]'
            ],
            'views:badge' => [
    ...
    protected function getEditorHref($filePath, $line) {
        if (empty(CF::config('debug.debugbar.editor'))) {
            return null;
        }

        if (empty($this->editors[CF::config('debug.debugbar.editor')])) {
            throw new InvalidArgumentException(
                'Unknown editor identifier: ' . CF::config('debug.debugbar.editor') . '. Known editors:'
        ...
        $url = str_replace(['%file', '%line'], [$filePath, $line], $this->editors[CF::config('debug.debugbar.editor')]);
     ...
     */
    public function addView(CView_View $view) {
        $name = $view->getName();
        $path = $view->getPath();

        if (!is_object($path)) {
            if ($path) {
                $path = ltrim(str_replace(DOCROOT, '', realpath($path)), '/');
            }
    ...
    protected function replaceSitesPath($filePath) {
        return str_replace(CF::config('debugbar.remote_sites_path'), CF::config('debugbar.local_sites_path'), $filePath);
    }

    public function getAssets() {
        return [
            'css' => ['debug/debugbar/widgets/templates/widget.css'],
            'js' => ['debug/debugbar/widgets/templates/widget.js',
                'debug/debugbar/widgets/view/widget.js']
        ];
    }
}
