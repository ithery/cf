...
class CBase_Sleep {
    ...
    public static function until($timestamp) {
        if (is_numeric($timestamp)) {
            $timestamp = CCarbon::createFromTimestamp($timestamp, date_default_timezone_get());
        }

        return new static(CCarbon::now()->diff($timestamp));
    }
    ...
    public function __destruct() {
        ...
        if (static::$fake) {
            ...
            if (static::$syncWithCarbon) {
                CCarbon::setTestNow(CCarbon::now()->add($this->duration));
            }
    ...
    protected function pullPending() {
        ...
        return c::tap($this->pending, function () {
            $this->pending = null;
        });
    }
    ...
    public static function assertSlept($expected, $times = 1) {
        $count = c::collect(static::$sequence)->filter($expected)->count();
    ...
    public static function assertSequence($sequence) {
        ...
        c::collect($sequence)
            ->zip(static::$sequence)
            ->eachSpread(function (?CBase_Sleep $expected, CarbonInterval $actual) {
                if ($expected === null) {
                    return;
                }
    ...
    public function when($condition) {
        $this->shouldSleep = (bool) c::value($condition, $this);
    ...
    public function unless($condition) {
        return $this->when(!c::value($condition, $this));
    }
