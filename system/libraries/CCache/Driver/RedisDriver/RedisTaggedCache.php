...
class CCache_Driver_RedisDriver_RedisTaggedCache extends CCache_TaggedCache {
    ...
    protected function pushKeys($namespace, $key, $reference) {
        $fullKey = $this->driver->getPrefix() . sha1($namespace) . ':' . $key;

        foreach (explode('|', $namespace) as $segment) {
            $this->driver->connection()->sadd($this->referenceKey($segment, $reference), $fullKey);
        }
    }
    ...
    protected function deleteKeysByReference($reference) {
        foreach (explode('|', $this->tags->getNamespace()) as $segment) {
            ...
            $this->driver->connection()->del($segment);
        }
    }
    ...
    protected function deleteValues($referenceKey) {
        $values = array_unique($this->driver->connection()->smembers($referenceKey));

        if (count($values) > 0) {
            foreach (array_chunk($values, 1000) as $valuesChunk) {
                call_user_func_array([$this->driver->connection(), 'del'], $valuesChunk);
            }
        }
    }
