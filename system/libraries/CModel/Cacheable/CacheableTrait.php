...
trait CModel_Cacheable_CacheableTrait {
    ...
    public static function bootCacheableEloquent() {
        static::updated(function (CModel $cachedModel) {
            !$cachedModel->isCacheClearEnabled() || $cachedModel::forgetCache();
        });
        static::created(function (CModel $cachedModel) {
            !$cachedModel->isCacheClearEnabled() || $cachedModel::forgetCache();
        });
        static::deleted(function (CModel $cachedModel) {
            !$cachedModel->isCacheClearEnabled() || $cachedModel::forgetCache();
        });
    }
     ...
     * @return void
     */
    protected static function storeCacheKey($modelName, $cacheKey) {
    ...
    public function getCacheLifetime() {
        return $this->cacheLifetime ? $this->cacheLifetime : -1;
    }
    ...
    public function isCacheClearEnabled() {
        return $this->cacheClearEnabled ? $this->cacheClearEnabled : true;
    }
    ...
    protected function generateCacheKey($builder, array $columns) {
        $query = $builder instanceof CModel_Query ? $builder->getQuery() : $builder;
        $vars = [
        ...
        return md5(json_encode([
            ...
            $builder instanceof CModel_Query ? $builder->getEagerLoads() : null,
            $builder->getBindings(),
            $builder->toSql(),
        ]));
    }
