...
class ModelInspector {
    ...
    protected function getVirtualAttributes($model, $columns) {
        ...
        return (new CCollection($class->getMethods()))
            ...
            ->reject(fn ($cast, $name) => (new CCollection($columns))->contains('name', $name))
            ->map(fn ($cast, $name) => [
                'name' => $name,
                'type' => null,
                'increments' => false,
    ...
    protected function getRelations($model) {
        return (new CCollection(get_class_methods($model)))
            ...
            ->filter(function (ReflectionMethod $method) {
                ...
                return (new CCollection($this->relationMethods))
    ...
    protected function getEvents($model) {
        return (new CCollection($model->dispatchesEvents()))
    ...
    protected function getObservers($model) {
        ...
        $listeners = array_filter($listeners, function ($v, $key) use ($model) {
            return cstr::startsWith($key, 'eloquent.') && cstr::endsWith($key, $model::class);
     ...
     * @param \Illuminate\Database\Eloquent\Model $model
     *
     * @return null|\Illuminate\Http\Resources\Json\JsonResource
     */
    protected function getResource($model) {
        return c::rescue(static fn () => $model->toResource()::class, null, false);
    }
