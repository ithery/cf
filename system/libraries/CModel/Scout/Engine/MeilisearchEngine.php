...
class CModel_Scout_Engine_MeilisearchEngine extends CModel_Scout_EngineAbstract implements CModel_Scout_Contract_UpdatesIndexSettingsInterface {
    /**
    ...
    public function delete($models) {
        ...
        $keys = $models instanceof CModel_Scout_RemoveableScoutCollection
     ...
     * @return mixed
     */
    protected function performSearch(CModel_Scout_Builder $builder, array $searchParams = []) {
     ...
     * @return string
     */
    protected function filters(CModel_Scout_Builder $builder) {
        $filters = c::collect($builder->wheres)
        ...
        foreach ($whereInOperators as $property => $operator) {
            if (property_exists($builder, $property)) {
                foreach ($builder->{$property} as $key => $values) {
                    $filters->push(sprintf('%s %s [%s]', $key, $operator, c::collect($values)->map(function ($value) {
                        if (is_bool($value)) {
     ...
     */
    protected function buildSortFromOrderByClauses(CModel_Scout_Builder $builder): array {
        return c::collect($builder->orders)
    ...
    public function mapIdsFrom($results, $key) {
        return count($results['hits']) === 0
            ? c::collect()
            : c::collect($results['hits'])->pluck($key)->values();
    }
    ...
    public function map(CModel_Scout_Builder $builder, $results, $model) {
        ...
        $objectIds = c::collect($results['hits'])->pluck($model->getScoutKeyName())->values()->all();
    ...
    public function lazyMap(CModel_Scout_Builder $builder, $results, $model) {
        if (count($results['hits']) === 0) {
            return CCollection_LazyCollection::make($model->newCollection());
        }
    ...
    public function createIndex($name, array $options = []) {
        ...
        if (c::optional($index)->getUid() !== null) {
            return $index;
        }
    ...
    public function updateIndexSettings($name, array $settings = []) {
        ...
        $index->updateSettings(carr::except($settings, 'embedders'));
