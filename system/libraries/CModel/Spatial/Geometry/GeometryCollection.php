...
class CModel_Spatial_Geometry_GeometryCollection extends CModel_Spatial_GeometryAbstract implements IteratorAggregate, ArrayAccess, CInterface_Arrayable, Countable {
     ...
     * @var string
     */
    protected $collectionItemType = CModel_Spatial_Contract_GeometryInterface::class;

    /**
     * The items contained in the spatial collection.
     *
     * @var CModel_Spatial_Contract_GeometryInterface[]
     */
    protected $items = [];

    /**
     * @param CModel_Spatial_Contract_GeometryInterface[] $geometries
     * @param int                                         $srid
     *
    ...
    public static function fromString($wktArgument, $srid = 0) {
        ...
        return new static(array_map(function ($geometry_string) {
            $klass = CModel_Spatial_GeometryAbstract::getWKTClass($geometry_string);
    ...
    public static function fromJson($geoJson) {
        ...
        if (!is_a($geoJson, FeatureCollection::class)) {
            throw new CModel_Spatial_Exception_InvalidGeoJsonException('Expected ' . FeatureCollection::class . ', got ' . get_class($geoJson));
        }
