...
class CDaemon_Supervisor_Repository_RedisMetricsRepository implements CDaemon_Supervisor_Contract_MetricsRepositoryInterface {
    ...
    public function measuredJobs() {
        ...
        return c::collect($classes)->map(function ($class) {
    ...
    public function measuredQueues() {
        ...
        return c::collect($queues)->map(function ($class) {
    ...
    public function throughput() {
        return c::collect($this->measuredQueues())->reduce(function ($carry, $queue) {
    ...
    public function queueWithMaximumRuntime() {
        return c::collect($this->measuredQueues())->sortBy(function ($queue) {
    ...
    protected function snapshotsFor($key) {
        return c::collect($this->connection()->zrange('snapshot:' . $key, 0, -1))
    ...
    public function snapshot() {
        c::collect($this->measuredJobs())->each(function ($job) {
            $this->storeSnapshotForJob($job);
        });

        c::collect($this->measuredQueues())->each(function ($queue) {
    ...
    protected function storeSnapshotForQueue($queue) {
        ...
        $this->connection()->zadd(
            ...
            json_encode([
                'throughput' => $data['throughput'],
                'runtime' => $data['runtime'],
                'wait' => CDaemon_Supervisor::waitTimeCalculator()->calculateFor($queue),
                'time' => $time,
            ])
        );
    ...
    protected function storeSnapshotTimestamp() {
        return c::tap(CarbonImmutable::now()->getTimestamp(), function ($timestamp) {
    ...
    public function acquireWaitTimeMonitorLock() {
        return CDaemon_Supervisor::lock()->get('monitor:time-to-clear');
    }
