 ...
 * WebSocket protocol.
 */
class CDaemon_Server_Protocol_Websocket extends CDaemon_Server_ProtocolAbstract {
    /**
     * Websocket blob type.
     *
     * @var string
     ...
     * @return int
     */
    public static function input($buffer, CDaemon_Server_ConnectionInterface $connection) {
        // Receive length.
        ...
        if ($connection->websocketCurrentFrameLength) {
            ...
            $masked = $secondbyte >> 7;
            if (!$masked) {
                CDaemon_Server::safeEcho("frame not masked so close the connection\n");
                $connection->close();
            ...
            switch ($opcode) {
                ...
                // Wrong opcode.
                default:
                    CDaemon_Server::safeEcho("error opcode $opcode and close websocket connection. Buffer:" . bin2hex($buffer) . "\n");
                    $connection->close();
            ...
            if ($total_package_size > $connection->maxPackageSize) {
                CDaemon_Server::safeEcho("error package. package_length=$total_package_size\n");
                $connection->close();
     ...
     * @return string
     */
    public static function encode($buffer, CDaemon_Server_ConnectionInterface $connection) {
        if (!is_scalar($buffer)) {
        ...
        if (empty($connection->websocketHandshake)) {
            ...
            if (strlen($connection->tmpWebsocketData) > $connection->maxSendBufferSize) {
                if ($connection->onError) {
                    try {
                        call_user_func($connection->onError, $connection, CDaemon_Server_Constant::SEND_FAIL, 'send buffer full and drop package');
                    } catch (\Exception $e) {
                        CDaemon_Server::log($e);
                        exit(250);
                    } catch (\Error $e) {
     ...
     * @return string
     */
    public static function decode($buffer, CDaemon_Server_ConnectionInterface $connection) {
        $masks = $data = $decoded = '';
     ...
     *
     * @param string                                  $buffer
     * @param CDaemon_Server_Connection_TcpConnection $connection
     *
     * @return int
     */
    protected static function dealHandshake($buffer, $connection) {
        ...
        if (0 === strpos($buffer, 'GET')) {
            ...
            if (!$has_server_header) {
                $handshake_message .= 'Server: workerman/' . CDaemon_Server_Constant::VERSION . "\r\n";
            }
            $handshake_message .= "\r\n";
