 ...
 * TcpConnection.
 */
class CDaemon_Server_Connection_TcpConnection extends CDaemon_Server_ConnectionAbstract {
    /**
     * Read buffer size.
     *
     * @var int
     ...
     *
     * @var null|string|CDaemon_Server_ProtocolInterface
     */
    ...
    /**
     * Which listener this connection belongs to.
     *
     * @var null|CDaemon_Server_ListenerAbstract
     */
    public $worker = null;
    ...
    public function __construct($socket, $remote_address = '') {
        ...
        $this->id = $this->internalId = self::$idRecorder++;
        ...
        CDaemon_Server::getEventLoop()->add($this->socket, CDaemon_Server_Constant::EV_READ, [$this, 'baseRead']);
    ...
    public function send($send_buffer, $raw = false) {
        ...
        if ($this->sendBuffer === '') {
            if ($this->transport === 'ssl') {
                CDaemon_Server::getEventLoop()->add($this->socket, CDaemon_Server_Constant::EV_WRITE, [$this, 'baseWrite']);
                $this->sendBuffer = $send_buffer;
                $this->checkBufferWillFull();
            ...
            if ($len > 0) {
                ...
                if (!is_resource($this->socket) || feof($this->socket)) {
                    ...
                    if ($this->onError) {
                        try {
                            call_user_func($this->onError, $this, CDaemon_Server_Constant::SEND_FAIL, 'client closed');
                        } catch (\Exception $e) {
                            CDaemon_Server::log($e);
                            exit(250);
                        } catch (\Error $e) {
    ...
    public function pauseRecv() {
        CDaemon_Server::getEventLoop()->del($this->socket, CDaemon_Server_Constant::EV_READ);
        $this->isPaused = true;
    }
    ...
    public function baseRead($socket, $check_eof = true) {
        ...
        if ($this->transport === 'ssl' && $this->sslHandshakeCompleted !== true) {
            if ($this->doSslHandshake($socket)) {
                ...
                if ($this->sendBuffer) {
                    CDaemon_Server::getEventLoop()->add($socket, CDaemon_Server_Constant::EV_WRITE, [$this, 'baseWrite']);
                }
            } else {
                return;
            }
            ...
            while ($this->recvBuffer !== '' && !$this->isPaused) {
                ...
                if ($this->currentPackageLength) {
                    ...
                    set_error_handler(function ($code, $msg, $file, $line) {
                        CDaemon_Server::safeEcho("$msg in file $file on line $line\n");
                    });
                    ...
                    if ($this->currentPackageLength === 0) {
                        ...
                        // Wrong package.
                        CDaemon_Server::safeEcho('error package. package_length=' . var_export($this->currentPackageLength, true));
                        $this->destroy();
    ...
    public function baseWrite() {
        ...
        if ($len === strlen($this->sendBuffer)) {
            $this->bytesWritten += $len;
            CDaemon_Server::getEventLoop()->del($this->socket, CDaemon_Server_Constant::EV_WRITE);
            $this->sendBuffer = '';
    ...
    public function doSslHandshake($socket) {
        ...
        $async = $this instanceof CDaemon_Server_Connection_AsyncTcpConnection;
        ...
        set_error_handler(function ($errno, $errstr, $file) {
            CDaemon_Server::safeEcho("SSL handshake error: $errstr \n");
        });
     ...
     *
     * @param CDaemon_Server_Connection_TcpConnection $dest
     *
     * @return void
     */
    ...
    protected function bufferIsFull() {
        ...
        if ($this->maxSendBufferSize <= strlen($this->sendBuffer)) {
            if ($this->onError) {
                try {
                    call_user_func($this->onError, $this, CDaemon_Server_Constant::SEND_FAIL, 'send buffer full and drop package');
                } catch (\Exception $e) {
                    CDaemon_Server::log($e);
                    exit(250);
                } catch (\Error $e) {
    ...
    public function destroy() {
        ...
        if ($this->status === self::STATUS_CLOSED) {
            ...
            if ($this->worker) {
                unset($this->worker->connections[$this->internalId]);
            }
            unset(static::$connections[$this->internalId]);
        }
    }
    ...
    public function __destruct() {
        static $mod;
        self::$statistics['connection_count']--;
        if (CDaemon_Server::getGracefulStop()) {
            if (!isset($mod)) {
                $mod = ceil((self::$statistics['connection_count'] + 1) / 3);
            }
            if (0 === self::$statistics['connection_count'] % $mod) {
                CDaemon_Server::log('worker[' . posix_getpid() . '] remains ' . self::$statistics['connection_count'] . ' connection(s)');
            }
            if (0 === self::$statistics['connection_count']) {
                CDaemon_Server::stopAll();
            }
        }
    }
}
