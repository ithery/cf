...
class CDatabase_Query_Grammar_PostgresGrammar extends CDatabase_Query_Grammar {
    /**
    ...
    public function whereFullText(CDatabase_Query_Builder $query, $where) {
        ...
        $columns = c::collect($where['columns'])->map(function ($column) use ($language) {
     ...
     * @return string
     */
    public function compileUpdateFrom(CDatabase_Query_Builder $query, $values) {
        ...
        if (isset($query->joins)) {
            ...
            $froms = c::collect($query->joins)->map(function ($join) {
     ...
     * @return string
     */
    protected function compileUpdateWheres(CDatabase_Query_Builder $query) {
     ...
     * @return string
     */
    protected function compileUpdateJoinWheres(CDatabase_Query_Builder $query) {
        $joinWheres = [];
    ...
    public function prepareBindingsForUpdateFrom(array $bindings, array $values) {
        $values = c::collect($values)->map(function ($value, $column) {
            return is_array($value) || ($this->isJsonSelector($column) && !$this->isExpression($value))
                ? json_encode($value)
                : $value;
        })->all();

        $bindingsWithoutWhere = carr::except($bindings, ['select', 'where']);

        return array_values(
            array_merge($values, $bindings['where'], carr::flatten($bindingsWithoutWhere))
        );
    }
    ...
    protected function wrapJsonPathAttributes($path) {
        ...
        return c::collect($path)->map(function ($attribute) {
    ...
    protected function parseJsonPathArrayKeys($attribute) {
        if (preg_match('/(\[[^\]]+\])+$/', $attribute, $parts)) {
            $key = cstr::beforeLast($attribute, $parts[0]);

            preg_match_all('/\[([^\]]+)\]/', $parts[0], $keys);

            return c::collect([$key])
                ->merge($keys[1])
                ->diff('')
                ->values()
                ->all();
