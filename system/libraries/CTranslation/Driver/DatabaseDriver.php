...
class CTranslation_Driver_DatabaseDriver extends CTranslation_DriverAbstract implements CTranslation_DriverInterface {
    ...
    public function allLanguages() {
        return CApp_Model_Language::all()->mapWithKeys(function ($language) {
    ...
    public function allGroup($language) {
        $groups = CApp_Model_Translation::getGroupsForLanguage($language);
    ...
    public function addLanguage($language, $name = null) {
        if ($this->languageExists($language)) {
            throw new CTranslation_Exception_LanguageExistsException(c::__('translation.errors_language_exists', ['language' => $language]));
        }

        CApp_Model_Language::create([
            'language' => $language,
            'name' => $name,
        ]);
    }
    ...
    public function addGroupTranslation($language, $group, $key, $value = '') {
        ...
        CApp_Model_Language::where('language', $language)
            ->first()
            ->translations()
            ->updateOrCreate([
                'group' => $group,
    ...
    public function getSingleTranslationsFor($language) {
        ...
        if ($this->hasLegacyGroups($translations->keys())) {
            CApp_Model_Translation::whereNull('group')->update(['group' => 'single']);
     ...
     * @param string $language
     *
     * @return CApp_Model_Language
     */
    private function getLanguage($language) {
        return CApp_Model_Language::where('language', $language)->first();
    }
