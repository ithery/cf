...
class CQueue_Middleware_WithoutOverlapping {
     ...
     *
     * @var null|\DateTimeInterface|int
     */
    public $releaseAfter;
     ...
     * @var string
     */
    public $prefix = 'cf-queue-overlap:';
     ...
     *
     * @param string                      $key
     * @param null|\DateTimeInterface|int $releaseAfter
    ...
    public function handle($job, $next) {
        $cache = c::cache()->driver();
        /** @var CCache_DriverAbstract $cache */
        $lock = $cache->lock(
            $this->getLockKey($job),
            $this->expiresAfter
        );
