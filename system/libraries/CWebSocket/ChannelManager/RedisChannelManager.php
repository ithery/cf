...
class CWebSocket_ChannelManager_RedisChannelManager extends CWebSocket_ChannelManager_LocalChannelManager {
    /**
     * The running loop.
     *
     * @var LoopInterface
    ...
    public function getChannelMembers($appId, $channel) {
        return $this->publishClient
            ...
            ->then(function ($list) {
                return c::collect(CWebSocket_Helper::redisListToArray($list))->map(function ($user) {
    ...
    public function removeObsoleteConnections() {
        $this->lock()->get(function () {
            $this->getConnectionsFromSet(0, c::now()->subMinutes(2)->format('U'))
     ...
     * @return void
     */
    public function onMessage($redisChannel, $payload) {
        ...
        $payload->channel = cstr::after($redisChannel, "{$payload->appId}:");

        if (!$channel = $this->find($payload->appId, $payload->channel)) {
            return;
        }

        $appId = isset($payload->appId) ? $payload->appId : null;
        $socketId = isset($payload->socketId) ? $payload->socketId : null;
        $serverId = isset($payload->serverId) ? $payload->serverId : null;

        CWebSocket_DashboardLogger::log($appId, CWebSocket_DashboardLogger::TYPE_REPLICATOR_MESSAGE_RECEIVED, [
    ...
    protected function getConnectionUri() {
        $name = CF::config('websocket.replication.modes.redis.connection', 'default');
        $config = CF::config("database.redis.{$name}");
     ...
     * @return PromiseInterface[int]
     */
    public function incrementSubscriptionsCount($appId, $channel = null, $increment = 1) {
     ...
     * @return PromiseInterface[int]
     */
    public function decrementSubscriptionsCount($appId, $channel = null, $increment = 1) {
     ...
     *
     * @param \Ratchet\ConnectionInterface $connection
     * @param null|\DateTime|string        $moment
     *
     * @return PromiseInterface
     */
     ...
     */
    public function getConnectionsFromSet($start = 0, $stop = 0, $strict = true) {
        if ($strict) {
            $start = "({$start}";
            $stop = "({$stop}";
        }
        ...
        return $this->publishClient
            ...
            ->then(function ($list) {
                return c::collect($list)->mapWithKeys(function ($appWithSocket) {
                    list($appId, $socketId) = explode(':', $appWithSocket);
     ...
     * @return PromiseInterface
     */
    public function addChannelToSet($appId, $channel) {
     ...
     * @return PromiseInterface
     */
    public function removeChannelFromSet($appId, $channel) {
     ...
     * @return PromiseInterface
     */
    public function storeUserData($appId, $channel = null, $key = null, $data = null) {
     ...
     * @return PromiseInterface
     */
    public function removeUserData($appId, $channel = null, $key = null) {
     ...
     * @return PromiseInterface
     */
    public function subscribeToTopic($appId, $channel = null) {
        $topic = $this->getRedisTopicName($appId, $channel);

        CWebSocket_DashboardLogger::log($appId, CWebSocket_DashboardLogger::TYPE_REPLICATOR_SUBSCRIBED, [
     ...
     * @return PromiseInterface
     */
    public function unsubscribeFromTopic($appId, $channel = null) {
        $topic = $this->getRedisTopicName($appId, $channel);

        CWebSocket_DashboardLogger::log($appId, CWebSocket_DashboardLogger::TYPE_REPLICATOR_UNSUBSCRIBED, [
            'serverId' => $this->getServerId(),
            'pubsubTopic' => $topic,
        ]);
     ...
     * @return PromiseInterface
     */
    protected function addUserSocket($appId, $channel, $user, $socketId) {
        return $this->publishClient->sadd(
     ...
     * @return PromiseInterface
     */
    protected function removeUserSocket($appId, $channel, $user, $socketId) {
        return $this->publishClient->srem(
            $this->getUserSocketsRedisHash($appId, $channel, $user->user_id),
            $socketId
        );
     ...
     * @return string
     */
    public function getRedisKey($appId = null, $channel = null, array $suffixes = []) {
        $prefix = CF::config('database.redis.options.prefix', null);
     ...
     * @return string
     */
    public function getStatsRedisHash($appId, $channel = null) {
     ...
     * @return string
     */
    public function getSocketsRedisHash() {
     ...
     * @return string
     */
    public function getChannelsRedisHash($appId) {
     ...
     * @return string
     */
    public function getUsersRedisHash($appId, $channel = null) {
     ...
     * @return string
     */
    public function getUserSocketsRedisHash($appId, $channel = null, $userId = null) {
     ...
     * @return string
     */
    public function getRedisTopicName($appId, $channel = null) {
    ...
    protected function lock() {
        return new CCache_Lock_RedisLock($this->redis, static::$lockName, 0);
    }
     ...
     */
    public function fakeConnectionForApp($appId, $socketId) {
        return new CWebSocket_Server_MockableConnection($appId, $socketId);
    }
}
