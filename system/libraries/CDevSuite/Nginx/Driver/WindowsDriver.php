...
/**
 * Description of WindowsDriver
 *
 * @author Hery
 */
class CDevSuite_Nginx_Driver_WindowsDriver extends CDevSuite_Nginx_DriverAbstract {
    ...
    public function installConfiguration() {
        ...
        $this->files->putAsUser(
                $this->path() . '/conf/nginx.conf', str_replace(['VALET_USER', 'VALET_HOME_PATH'], [user(), VALET_HOME_PATH], $contents)
        );
    }
    ...
    public function installServer() {
        ...
        $this->files->putAsUser(
                $this->path() . '/valet/valet.conf', str_replace(
                        ['VALET_HOME_PATH', 'VALET_SERVER_PATH', 'VALET_STATIC_PREFIX', 'HOME_PATH'], [VALET_HOME_PATH, VALET_SERVER_PATH, VALET_STATIC_PREFIX, $_SERVER['HOME']], $this->files->get(__DIR__ . '/../stubs/valet.conf')
                )
        );

        $this->files->putAsUser(
                $this->path() . '/conf/fastcgi_params', $this->files->get(__DIR__ . '/../stubs/fastcgi_params')
        );
    }
