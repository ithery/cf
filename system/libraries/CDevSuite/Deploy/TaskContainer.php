...
class CDevSuite_Deploy_TaskContainer {
    ...
    public function getServer($server) {
        ...
        return carr::get($this->servers, $server);
    }
    ...
    public function import($file, array $data = []) {
        ...
        $this->load($path, new CDevSuite_Deploy_Compiler, $data);
    }
    ...
    protected function resolveImportPath($file) {
        if (($path = realpath($file)) !== false) {
            ...
            return $path;
        } elseif (($path = realpath(getcwd() . '/vendor/' . $file . '/deploy.blade.php')) !== false) {
            return $path;
    ...
    public function getMacro($macro) {
        return carr::get($this->macros, $macro);
    }
    ...
    public function getMacroOptions($macro) {
        return carr::get($this->macroOptions, $macro, []);
    }
    ...
    public function getTask($task, array $macroOptions = []) {
        $script = carr::get($this->tasks, $task, '');
        ...
        $parallel = carr::get($options, 'parallel', false);

        $confirm = carr::get($options, 'confirm', null);

        return new CDevSuite_Deploy_Task($this->getServers($options), $options['as'], $script, $parallel, $confirm);
    }
    ...
    public function getTaskOptions($task) {
        return carr::get($this->taskOptions, $task, []);
    }
