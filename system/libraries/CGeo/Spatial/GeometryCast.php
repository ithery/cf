...
class CGeo_Spatial_GeometryCast implements CModel_Contract_CastsAttributesInterface {
    ...
    private function extractWktFromExpression(CDatabase_Query_Exp $expression, CDatabase_Connection $connection): string {
    ...
    private function extractSridFromExpression(ExpressionContract $expression, CDatabase_Connection $connection): int {
        $grammar = $connection->getQueryGrammar();
        $expressionValue = $expression->getValue($grammar);
