...
class CGeo_Provider_GeoIP2_Adapter {
    ...
    public function getContent($url) {
        if (false === filter_var($url, FILTER_VALIDATE_URL)) {
            throw new CGeo_Exception_InvalidArgument(
                sprintf('"%s" must be called with a valid url. Got "%s" instead.', __METHOD__, $url)
            );
        }
        $ipAddress = parse_url($url, PHP_URL_QUERY);
        if (false === filter_var($ipAddress, FILTER_VALIDATE_IP)) {
            throw new CGeo_Exception_InvalidArgument('URL must contain a valid query-string (an IP address, 127.0.0.1 for instance)');
        }
