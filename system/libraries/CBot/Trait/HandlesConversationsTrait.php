...
trait CBot_Trait_HandlesConversationsTrait {
     ...
     */
    public function startConversation(CBot_Message_ConversationAbstract $instance, $recipient = null, $driver = null) {
        if (!is_null($recipient) && !is_null($driver)) {
            ...
            $this->driver = CBot_DriverManager::loadFromName($driver, $this->config);
        }
    ...
    public function loadActiveConversation() {
        ...
        c::collect($this->getMessages())->reject(function (CBot_Message_Incoming_IncomingMessage $message) {
            return $message->isFromBot();
        })->filter(function (CBot_Message_Incoming_IncomingMessage $message) {
     ...
     */
    protected function callConversation($next, $convo, CBot_Message_Incoming_IncomingMessage $message, array $parameters) {
        /** @var \CBot_Message_ConversationAbstract $conversation */
        $conversation = $convo['conversation'];
        if (!$conversation instanceof CQueue_ShouldQueueInterface) {
            $conversation->setBot($this);
        }
        /*
         * Validate askForImages, askForAudio, etc. calls
         */
        $additionalParameters = c::collect(unserialize($convo['additionalParameters']));
    ...
    /**
     * @param Closure                           $next
     * @param CBot_Message_ConversationAbstract $conversation
     * @param array                             $parameters
     */
