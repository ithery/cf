...
trait CConsole_Prompt_Themes_Default_Concerns_InteractsWithStrings {
    /**
     ...
     * @return int
     */
    protected function longest(array $lines, $padding = 0) {
        $widths = array_map(function ($line) use ($padding) {
            return mb_strwidth($this->stripEscapeSequences($line)) + $padding;
        }, $lines);

        return max(
            $this->minWidth,
            count($lines) > 0 ? max($widths) : null
        );
    }
     ...
     * @return string
     */
    protected function pad($text, $length, $char = ' ') {
     ...
     * @return string
     */
    protected function mbWordwrap($string, $width = 75, $break = "\n", $cut_long_words = false) {
     ...
     */
    protected function ansiWordwrap($text, $width) {
        ...
        foreach ($plainLines as $plainLine) {
            ...
            if ($lastCodes !== '' && !cstr::endsWith($line, "\e[0m")) {
                $line .= "\e[0m";
            }
     ...
     */
    protected function parseAnsiText($text) {
        $segments = [];
        $currentCodes = '';
        $currentLink = '';
        $currentText = '';
