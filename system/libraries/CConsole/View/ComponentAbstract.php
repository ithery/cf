...
abstract class CConsole_View_ComponentAbstract {
    /**
    ...
    protected function compile($view, $data) {
        ...
        include DOCROOT . "system/views/cresenity/console/views/components/$view.php";

        return c::tap(ob_get_contents(), function () {
            ob_end_clean();
        });
    }
    ...
    protected function usingQuestionHelper($callable) {
        $property = c::with(new ReflectionClass(CConsole_OutputStyle::class))
            ->getParentClass()
        ...
        $property->setValue($this->output, new CConsole_QuestionHelper());
