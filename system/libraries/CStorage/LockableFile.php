...
class CStorage_LockableFile {
     ...
     * @param bool $block
     *
     * @throws \CStorage_Exception_LockTimeoutException
     *
     * @return $this
     */
    public function getSharedLock($block = false) {
        if (!flock($this->handle, LOCK_SH | ($block ? 0 : LOCK_NB))) {
            throw new CStorage_Exception_LockTimeoutException("Unable to acquire file lock at path [{$this->path}].");
        }
