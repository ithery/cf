...
class CVendor_OneBrick_Client {
    /**
     * @var CVendor_OneBrick_Contract_AdapterInterface
     */
    protected $adapter;

    protected $baseUri;

    public function __construct(CVendor_OneBrick_Contract_AdapterInterface $adapter, $baseUri) {
        $this->adapter = $adapter;
        $this->baseUri = $baseUri;
    }

    public function url($method, $queries = null) {
        $url = $this->baseUri . '/' . ltrim($method, '/');
        $queryString = '';
        if ($queries != null) {
