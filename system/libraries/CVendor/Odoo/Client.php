...
class CVendor_Odoo_Client {
    ...
    public function getUserId() {
        ...
        $this->serverVersion = isset($version['server_version']) ? $version['server_version'] : '';
        if ($this->userId > 0) {
            return $this->userId;
        }
    ...
    public function stringValue($data) {
    ...
    public function booleanValue($data) {
        return $this->value($data, static::TYPE_BOOLEAN);
    }

    public function doubleValue($data) {
     ...
     */
    public function unlink($modelName, $resourceId) {
     ...
     */
    public function fieldsGet($modelName) {
    ...
    public function load($modelName, iterable $records) {
        ...
        foreach ($groups as $group) {
            ...
            if (is_array(isset($groupResult['ids']) ? $groupResult['ids'] : null)) {
                $ids = $ids->merge($groupResult['ids']);
            }
            if (is_array(isset($groupResult['messages']) ? $groupResult['messages'] : null)) {
    ...
    protected function mapModelName($modelName) {
        ...
        // Default fallback.
        return CVendor_Odoo_Model::class;
    }
     ...
     * @param null|string $action    will be used only the $modelName is provided
     *
     * @return Request
     */
    ...
    public function nativeToValue($item) {
        ...
        if ($item === [] || (is_array($item) && array_keys($item) === range(0, count($item) - 1)) || (carr::isIterable($item) && !is_array($item))
        ) {
