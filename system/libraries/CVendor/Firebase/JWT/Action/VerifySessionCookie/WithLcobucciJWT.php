 ...
 * @internal
 */
final class CVendor_Firebase_JWT_Action_VerifySessionCookie_WithLcobucciJWT implements CVendor_Firebase_JWT_Action_VerifySessionCookie_HandlerInterface {
    /**
     * @var string
     */
    private $projectId;
     ...
     */
    public function __construct($projectId, CVendor_Firebase_JWT_Contract_KeysInterface $keys, ClockInterface $clock) {
    ...
    public function handle(CVendor_Firebase_JWT_Action_VerifySessionCookie $action) {
        ...
        try {
            ...
            throw CVendor_Firebase_JWT_Exception_SessionCookieVerificationFailedException::withSessionCookieAndReasons($cookieString, ['The token is invalid', $e->getMessage()]);
        }
        ...
        if (!empty($errors)) {
            throw CVendor_Firebase_JWT_Exception_SessionCookieVerificationFailedException::withSessionCookieAndReasons($cookieString, $errors);
        }
        ...
        return CVendor_Firebase_JWT_Token::withValues($cookieString, $headers, $claims);
    }
    ...
    private function getKey(UnencryptedToken $token) {
        if (empty($keys = $this->keys->all())) {
            throw CVendor_Firebase_JWT_Exception_SessionCookieVerificationFailedException::withSessionCookieAndReasons($token->toString(), ["No keys are available to verify the token's signature."]);
        }
        ...
        throw CVendor_Firebase_JWT_Exception_SessionCookieVerificationFailedException::withSessionCookieAndReasons($token->toString(), ["No public key matching the key ID '{$keyId}' was found to verify the signature of this session cookie."]);
    }
     ...
     * @return void
     */
    private function assertUserAuthedAt(UnencryptedToken $token, DateTimeInterface $now) {
