...
class Profiler {
    ...
    public function __construct() {
        ...
        CFEvent::add('profiler.run', array($this, 'benchmarks'));
        CFEvent::add('profiler.run', array($this, 'database'));
        CFEvent::add('profiler.run', array($this, 'session'));
        CFEvent::add('profiler.run', array($this, 'post'));
        CFEvent::add('profiler.run', array($this, 'cookies'));

        // Add profiler to page output automatically
        CFEvent::add('system.display', array($this, 'render'));

        CF::log('debug', 'Profiler Library initialized');
    }
    ...
    public function disable() {
        ...
        CFEvent::clear('system.display', array($this, 'render'));
    }
    ...
    public function render($return = FALSE) {
        ...
        $this->show = empty($get) ? CF::config('profiler.show') : $get;

        CFEvent::run('profiler.run', $this);
        ...
        $view = CView::factory('kohana_profiler', $data);
        ...
        if (stripos(CF::$output, '</body>') !== FALSE) {
            // Closing body tag was found, insert the profiler data before it
            CF::$output = str_ireplace('</body>', $view->render() . '</body>', CF::$output);
        } else {
            // Append the profiler data to the output
            CF::$output .= $view->render();
        }
    }
