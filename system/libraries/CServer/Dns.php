...
class CServer_Dns {
    ...
    /**
     * @var int default value 53
     */
    protected $port;

    /**
     * @var float default value DEFAULT_TIMEOUT | 60
     */
    protected $timeout;
    ...
    public function __construct($logging = false) {
        ...
        $this->udp = false;
        $this->types = new CServer_Dns_Types();
     ...
     * @param string $type
     *
     * @throws CServer_Dns_Exception
     *
     * @return CServer_Dns_Response
     */
    public function query($question, $type = 'A') {
        $typeid = $this->types->getByName($type);
        if ($typeid === false) {
            throw new CServer_Dns_Exception('Invalid Query Type ' . $type);
        }
        ...
        if (($this->udp) && ($headersize >= 512)) {
            ...
            throw new CServer_Dns_Exception('Question too big for UDP (' . $headersize . ' bytes)');
        }
        ...
        if ($this->udp) { // UDP method
            if (!fwrite($socket, $header, $headersize)) {
                ...
                throw new CServer_Dns_Exception('Failed to write question to socket');
            }
            if (!$this->rawbuffer = fread($socket, 4096)) { // read until the end with UDP
                fclose($socket);

                throw new CServer_Dns_Exception('Failed to write read data buffer');
            }
        } else { // TCP method
            if (!fwrite($socket, $headersizebin)) { // write the socket
                fclose($socket);

                throw new CServer_Dns_Exception('Failed to write question length to TCP socket');
            }
            if (!fwrite($socket, $header, $headersize)) {
                fclose($socket);

                throw new CServer_Dns_Exception('Failed to write question to TCP socket');
            }
            ...
            if (!$this->rawbuffer = fread($socket, $datasize)) {
                ...
                throw new CServer_Dns_Exception('Failed to read data buffer');
            }
        }
        fclose($socket);
        ...
        if ($buffersize < 12) {
            throw new CServer_Dns_Exception('DNS query return buffer too small');
        }
        ...
        $flags = sprintf("%016b\n", $this->header['flags']);
        $response = new CServer_Dns_Response();
        ...
        for ($a = 0; $a < $this->header['ancount']; $a++) {
            $response->ReadRecord($this->rawbuffer, CServer_Dns_Response::RESULTTYPE_RESOURCE);
        }
        ...
        for ($a = 0; $a < $this->header['nscount']; $a++) {
            $response->ReadRecord($this->rawbuffer, CServer_Dns_Response::RESULTTYPE_NAMESERVER);
        }
        ...
        for ($a = 0; $a < $this->header['arcount']; $a++) {
            $response->ReadRecord($this->rawbuffer, CServer_Dns_Response::RESULTTYPE_ADDITIONAL);
        }
    ...
    public function setTimeout($timeout = self::DEFAULT_TIMEOUT) {
        if (!$new_timeout = floatval($timeout) || $timeout < 0) {
            throw new CServer_Dns_Exception('Incorrect timeout value: <' . $timeout . '>. Timeout must be positive number.');
        }
