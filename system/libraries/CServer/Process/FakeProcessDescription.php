...
class CServer_Process_FakeProcessDescription {
    /**
     * The process' ID.
     *
     * @var null|int
    ...
    public function output($output) {
        if (is_array($output)) {
            c::collect($output)->each(fn ($line) => $this->output($line));
    ...
    public function errorOutput($output) {
        if (is_array($output)) {
            c::collect($output)->each(fn ($line) => $this->errorOutput($line));
    ...
    public function replaceOutput(string $output) {
        $this->output = c::collect($this->output)->reject(function ($output) {
    ...
    public function iterations(int $iterations) {
        return $this->runsFor($iterations);
    }
