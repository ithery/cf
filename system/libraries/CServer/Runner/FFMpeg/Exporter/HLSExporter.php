...
class CServer_Runner_FFMpeg_Exporter_HLSExporter extends CRunner_FFMpeg_Exporter_MediaExporter {
    ...
    private function prepareSaving(string $path = null): Collection {
        ...
        return $this->pendingFormats->map(function (array $formatAndCallback, $key) use ($baseName, $media) {
            list($format, $filtersCallback) = $formatAndCallback;

            list($segmentsPattern, $formatPlaylistPath) = $this->getSegmentPatternAndFormatPlaylistPath(
                $baseName,
                $format,
                $key
            );
