...
class CServer_Dns_Response {
    ...
    public function addResult($result, $recordtype) {
        switch ($recordtype) {
            case CServer_Dns_Response::RESULTTYPE_RESOURCE:
                $this->resourceResults[] = $result;

                break;
            case CServer_Dns_Response::RESULTTYPE_NAMESERVER:
                $this->nameserverResults[] = $result;

                break;
            case CServer_Dns_Response::RESULTTYPE_ADDITIONAL:
    ...
    public function readRecord($buffer, $resulttype = '') {
        ...
        $types = new CServer_Dns_Types();
        $typeid = $types->getById($ans_header['type']);
        //$extras = array();
        switch ($typeid) {
            case 'A':
                $result = new CServer_Dns_Result_AResult(implode('.', unpack('Ca/Cb/Cc/Cd', $this->readResponse($buffer, 4))));

                break;

            case 'AAAA':
                // short: 2a01:448:1004::11
                $result = new CServer_Dns_Result_AAAAResult(implode(':', unpack('H4a/H4b/H4c/H4d/H4e/H4f/H4g/H4h', $this->readResponse($buffer, 16))));

                break;

            case 'NS':
                $result = new CServer_Dns_Result_NSResult($this->readDomainLabel($buffer));

                break;

            case 'PTR':
                $result = new CServer_Dns_Result_PTRResult($this->readDomainLabel($buffer));

                break;

            case 'CNAME':
                $result = new CServer_Dns_Result_CNAMEResult($this->readDomainLabel($buffer));

                break;

            case 'MX':
                $result = new CServer_Dns_Result_MXResult();
            ...
            case 'SOA':
                $result = new CServer_Dns_Result_SOAResult();
            ...
            case 'TXT':
                $result = new CServer_Dns_Result_TXTResult($this->readResponse($buffer, $ans_header['length']));
            ...
            case 'DS':
                ...
                $result = new CServer_Dns_Result_DSResult($stuff['keytag'], $stuff['algo'], $stuff['digest'], $stuff['string'], $stuff['rest']);
            ...
            case 'DNSKEY':
                ...
                $flags = sprintf("%016b\n", $extras['flags']);
                $result = new CServer_Dns_Result_DNSKEYResult($extras['flags'], $extras['protocol'], $extras['algorithm'], $extras['pubkey']);
            ...
            case 'RRSIG':
                ...
                $result = new CServer_Dns_Result_RRSIGResult($test['type'], $test['algorithm'], $test['labels'], $test['originalttl'], $test['expiration'], $test['inception'], $test['keytag']);
