...
class CServer_NodeJs_Wrapper_ReactWrapper_ReactWrapperFallback {
    protected $source;
    ...
    protected function startWithTag($buffer, &$matches) {
        ...
        if (preg_match('/^<(' . $start . $middle . '*)(\s|\\/|>)/', $buffer, $tagMatches)) {
            ...
            if (preg_match('/^<' . $tagName . $this->attributesPattern . '\\/>/', $buffer, $matches)
    ...
    public function parseJsx() {
        ...
        while (mb_strlen($this->buffer)) {
            ...
            $consume = null;
            if (preg_match('/^\/\*([\s\S]+)\*\//U', $this->buffer, $matches)
