...
class CBroadcast_Broadcaster_AblyBroadcaster extends CBroadcast_BroadcasterAbstract {
    /**
    ...
    public function normalizeChannelName($channel) {
        if ($this->isGuardedChannel($channel)) {
            return cstr::startsWith($channel, 'private-')
                        ? cstr::replaceFirst('private-', '', $channel)
                        : cstr::replaceFirst('presence-', '', $channel);
        }
    ...
    protected function formatChannels(array $channels) {
        return array_map(function ($channel) {
            ...
            if (cstr::startsWith($channel, ['private-', 'presence-'])) {
                return cstr::startsWith($channel, 'private-')
                    ? cstr::replaceFirst('private-', 'private:', $channel)
                    : cstr::replaceFirst('presence-', 'presence:', $channel);
            }
    ...
    protected function getPublicToken() {
        return cstr::before($this->ably->options->key, ':');
    }
    ...
    protected function getPrivateToken() {
        return cstr::after($this->ably->options->key, ':');
    }
