...
class CEmail_Client_Imap_BodyStructure {
    /**
     * @var string
     */
    ...
    public function isImage() {
        return 'image' === CEmail_Client_Utils::contentTypeType($this->contentType(), $this->fileName());
    }
    ...
    public function isArchive() {
        return 'archive' === CEmail_Client_Utils::contentTypeType($this->contentType(), $this->fileName());
    }
    ...
    public function isPdf() {
        return 'pdf' === \CEmail_Client_Utils::ContentTypeType($this->contentType(), $this->fileName());
    }
    ...
    public function isDoc() {
        return 'doc' === \CEmail_Client_Utils::ContentTypeType($this->contentType(), $this->fileName());
    }
    ...
    public function searchByCallback($fCallback) {
        ...
        if (\is_array($this->aSubParts) && 0 < \count($this->aSubParts)) {
            foreach ($this->aSubParts as /* @var $oSubPart \CEmail_Client_Imap_BodyStructure */ &$oSubPart) {
     ...
     *
     * @return \CEmail_Client_Imap_BodyStructure
     */
    public function getPartByMimeIndex($sMimeIndex) {
        ...
        if (0 < \strlen($sMimeIndex)) {
            ...
            if (null === $oPart && is_array($this->aSubParts) && 0 < count($this->aSubParts)) {
                foreach ($this->aSubParts as &$oSubPart) {
                    /** @var CEmail_Client_Imap_BodyStructure $oSubPart */
     ...
     * @param string $sCharset   = \CEmail_Client_Utils::CHARSET_UTF_8
     *
     * @return string
     */
    private static function decodeAttrParamenter($aParams, $sParamName, $sCharset = \CEmail_Client_Utils::CHARSET_UTF_8) {
        $sResult = '';
        if (isset($aParams[$sParamName])) {
            $sResult = \CEmail_Client_Utils::DecodeHeaderValue($aParams[$sParamName], $sCharset);
        } elseif (isset($aParams[$sParamName . '*'])) {
            $aValueParts = \explode('\'\'', $aParams[$sParamName . '*'], 2);
            if (\is_array($aValueParts) && 2 === \count($aValueParts)) {
                $sCharset = isset($aValueParts[0]) ? $aValueParts[0] : \CEmail_Client_Utils::CHARSET_UTF_8;
    ...
    public static function newInstance(array $aBodyStructure, $sPartID = '') {
        if (!\is_array($aBodyStructure) || 2 > \count($aBodyStructure)) {
            ...
            return new self(
                ...
                $sDisposition,
                $aDispositionParams,
                \CEmail_Client_Utils::utf8Clear((null === $sFileName || 0 === \strlen($sFileName)) ? $sName : $sFileName),
                $sLanguage,
                $sLocation,
                $iSize,
                $iTextLineCount,
