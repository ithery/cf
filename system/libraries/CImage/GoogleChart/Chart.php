...
class CImage_GoogleChart_Chart {
    ...
    protected function encodeData($data, $separator, $encodigData = '') {
        if ($encodigData == 's') {
            ...
            $separator = '';
        } elseif ($encodigData == 'e') {
            $data = $this->extendedEncodeData($data);
            $separator = '';
        } elseif ($encodigData == 't') {
    ...
    /**
     * Encodes the data as Simple Text.
    ...
    /**
     * Specifies the style of an axis.
     *
    ...
    public function addAxisRange($axisIndex, $startVal, $endVal, $step = null) {
        if (is_null($step)) {
            $axisRange = [$axisIndex, $startVal, $endVal];
        } else {
            $axisRange = [$axisIndex, $startVal, $endVal, $step];
        }
    ...
    /**
     * Specifies the label positions on each axis independently.
     *
     ...
     *
     * @param int   $axisIndex      This is a zero-based index into the axis array specified by setVisibleAxes()
     * @param array $labelPositions The position of the label along the axis. This is a comma-separated list of
     ...
     * @param string $color    The fill color, in RRGGBB hexadecimal format. For transparencies, the first six digits are ignored,
     *                         but must be included anyway.
     */
    public function addBackgroundFill($fillType, $color) {
        $this->setProperty('chf', $this->encodeData([$fillType, 's', $color], ','), true);
    }
    ...
    public function setGradientFill($fillType, $fillAngle, $colors) {
        $this->setProperty('chf', $this->encodeData(array_merge([$fillType, 'lg', $fillAngle], $colors), ','));
    }
    ...
    public function setStripFill($fillType, $fillAngle, $colors) {
        $this->setProperty('chf', $this->encodeData(array_merge([$fillType, 'ls', $fillAngle], $colors), ','));
    }

    /**
     * Fills the area below a data line with a solid color.
     *
     * @param string $where          Whether to fill to the bottom of the chart, or just to the next lower line.
     *                               Must be B or b. Please refer to the documentation for the acceptable values
     * @param string $color          An RRGGBB format hexadecimal number of the fill color
     ...
     * @param int $yAxisStepSize Used to calculate how many x or y grid lines to show on the chart.
     *                           100 / step_size = how many grid lines on the chart.
