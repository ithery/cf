...
class CApi_Dispatcher {
     ...
     * @param \CApi_Routing_Router $router
     * @param \CApi_Auth           $auth
     *
     * @return void
     */
    public function __construct($apiGroup, CApi_Routing_Router $router, CApi_Auth $auth) {
     ...
     * @param string|array $parameters
     *
     * @return \CApi_Http_InternalRequest
     */
    protected function createRequest($verb, $uri, $parameters) {
        ...
        $request = CApi_HTTP_InternalRequest::create(
            $uri,
            $verb,
            $parameters,
            $this->cookies,
            $this->uploads,
            CHTTP::request()->server->all(),
            $this->content
        );
        ...
        $request->setApiGroup($this->apiGroup);
    ...
    protected function addPrefixToUri($uri) {
        ...
        if (cstr::startsWith($uri, $this->prefix)) {
            return $uri;
        }
     ...
     *
     * @param \CApi_Http_InternalRequest $request
     *
     * @throws \Exception|\Symfony\Component\HttpKernel\Exception\HttpExceptionInterface
     *
     * @return mixed
     */
    protected function dispatch(CApi_Http_InternalRequest $request) {
        ...
        try {
            ...
            if (!$response->isSuccessful() && !$response->isRedirection()) {
                throw new CApi_Exception_InternalHttpException($response);
            }
    ...
    protected function replaceRequestInstance() {
        ...
        CHTTP::setRequest(end($this->requestStack));
    }
