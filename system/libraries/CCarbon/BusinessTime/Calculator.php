...
class CCarbon_BusinessTime_Calculator {
    /**
     * @var CarbonInterface
     */
    protected $date;
    ...
    public function __construct(CarbonInterface $date, CarbonInterval $interval, bool $open, bool $holidaysAreClosed) {
        ...
        try {
            $this->openingHours = $date->getOpeningHours(CCarbon_BusinessTime::LOCAL_MODE);
     ...
     */
    public function calculate($maximum = INF) {
        ...
        for ($i = 0; $i < $maximum; $i++) {
            list($next, $resultCandidate) = $this->getNextAndCandidate($base, $remainingInterval);
