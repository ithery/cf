...
class CTesting_Browser_ElementResolver {
    ...
    protected function findButtonByText($button) {
        foreach ($this->all('button') as $element) {
            if (cstr::contains($element->getText(), $button)) {
                return $element;
            }
        }
    }
    ...
    public function format($selector) {
        $sortedElements = c::collect($this->elements)->sortByDesc(function ($element, $key) {
            return strlen($key);
        })->toArray();
        ...
        if (cstr::startsWith($selector, '@') && $selector === $originalSelector) {
