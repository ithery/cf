...
class AssertableJsonString implements ArrayAccess, Countable {
     ...
     *
     * @var CInterface_Jsonable|\JsonSerializable|array
     */
    public $json;
    ...
    public function assertSimilar(array $data) {
        $actual = json_encode(carr::sortRecursive(
            (array) $this->decoded
        ));

        PHPUnit::assertEquals(json_encode(carr::sortRecursive($data)), $actual);
    ...
    public function assertFragment(array $data) {
        ...
        foreach (carr::sortRecursive($data) as $key => $value) {
            $expected = $this->jsonSearchStrings($key, $value);

            PHPUnit::assertTrue(
                cstr::contains($actual, $expected),
    ...
    public function assertMissing(array $data, $exact = false) {
        ...
        foreach (carr::sortRecursive($data) as $key => $value) {
            ...
            PHPUnit::assertFalse(
                cstr::contains($actual, $unexpected),
    ...
    public function assertMissingExact(array $data) {
        ...
        foreach (carr::sortRecursive($data) as $key => $value) {
            ...
            if (!cstr::contains($actual, $unexpected)) {
                return $this;
            }
        }
