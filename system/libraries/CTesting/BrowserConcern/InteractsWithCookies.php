...
trait CTesting_BrowserConcern_InteractsWithCookies {
    ...
    public function cookie($name, $value = null, $expiry = null, array $options = []) {
        ...
        if ($cookie) {
            $decryptedValue = c::decrypt(rawurldecode($cookie['value']), $unserialize = false);

            $hasValuePrefix = strpos($decryptedValue, CHTTP_Cookie_CookieValuePrefix::create($name, CCrypt::encrypter()->getKey())) === 0;

            return $hasValuePrefix ? CHTTP_Cookie_CookieValuePrefix::remove($decryptedValue) : $decryptedValue;
        }
    }
    ...
    public function addCookie($name, $value, $expiry = null, array $options = [], $encrypt = true) {
        if ($encrypt) {
            $value = c::encrypt($value, $serialize = false);
        }
