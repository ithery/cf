...
trait CTesting_BrowserConcern_InteractsWithAuthentication {
    ...
    public function loginAs($userId, $guard = null) {
        ...
        return $this->visit(rtrim(c::route('dusk.login', ['userId' => $userId, 'guard' => $guard], $this->shouldUseAbsoluteRouteForAuthentication())));
    }
    ...
    public function logout($guard = null) {
        return $this->visit(rtrim(c::route('dusk.logout', ['guard' => $guard], $this->shouldUseAbsoluteRouteForAuthentication()), '/'));
    }
    ...
    protected function currentUserInfo($guard = null) {
        $response = $this->visit(c::route('dusk.user', ['guard' => $guard], $this->shouldUseAbsoluteRouteForAuthentication()));
    ...
    private function shouldUseAbsoluteRouteForAuthentication() {
        return CF::config('dusk.domain') !== null;
    }
}
