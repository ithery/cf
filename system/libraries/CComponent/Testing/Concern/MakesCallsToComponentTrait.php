...
trait CComponent_Testing_Concern_MakesCallsToComponentTrait {
    ...
    public function syncInput($name, $value) {
        if ($value instanceof CHTTP_UploadedFile) {
            return $this->syncUploadedFiles($name, [$value]);
        } elseif (is_array($value) && isset($value[0]) && $value[0] instanceof CHTTP_UploadedFile) {
    ...
    public function syncUploadedFiles($name, $files, $isMultiple = false) {
        ...
        $this->sendMessage('callMethod', [
            'method' => 'startUpload',
            'params' => [$name, c::collect($files)->map(function ($file) {
                return [
                    'name' => $file->name,
                    'size' => $file->getSize(),
        ...
        $newFileHashes = c::collect($files)->zip($fileHashes)->mapSpread(function ($file, $fileHash) {
            return (string) c::str($fileHash)->replaceFirst('.', "-size={$file->getSize()}.");
        })->toArray();

        c::collect($fileHashes)->zip($newFileHashes)->mapSpread(function ($fileHash, $newFileHash) use ($storage) {
            $storage->move('/' . CComponent_FileUploadConfiguration::path($fileHash), '/' . CComponent_FileUploadConfiguration::path($newFileHash));
        });
