...
class CValidation_ConditionalRules {
    /**
    ...
    public function passes(array $data = []) {
        return is_callable($this->condition)
                    ? call_user_func($this->condition, new CBase_Fluent($data))
                    : $this->condition;
    }
    ...
    public function rules(array $data = []) {
        return is_string($this->rules)
                    ? explode('|', $this->rules)
                    : c::value($this->rules, new CBase_Fluent($data));
    }
    ...
    public function defaultRules(array $data = []) {
        return is_string($this->defaultRules)
                    ...
                    : c::value($this->defaultRules, new CBase_Fluent($data));
    }
}
