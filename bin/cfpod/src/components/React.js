...
class React {
    ...
    register() {
        if (
            ...
            throw new Error(
                'cf.react() is now a feature flag. Use cf.js(source, destination).react() instead'
            );
        }
    }
    ...
    babelConfig() {
        const plugins = this.supportsFastRefreshing()
            ? [CF.resolve('react-refresh/babel')]
            : [];
    ...
    supportsFastRefreshing() {
        return CF.isHot() && semver.satisfies(this.library().version, '>=16.9.0');
    }
