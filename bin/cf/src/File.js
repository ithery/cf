...
class File {
    ...
    normalizedOutputPath() {
        let path = this.pathFromPublic(this.cf.config.publicPath);
     ...
     */
    relativePath() {
        return path.relative(this.cf.paths.root(), this.path());
    }
    ...
    relativePathWithoutExtension() {
        return path.relative(this.cf.paths.root(), this.pathWithoutExtension());
    }
    ...
    forceFromPublic(publicPath) {
        publicPath = publicPath || this.cf.config.publicPath;
    ...
    static stripPublicDir(filePath, publicPath = null) {
        let publicDir = path.basename(publicPath || this.cf.config.publicPath);
    ...
    pathFromPublic(publicPath) {
        ...
        return path.normalize(
            '/' + path.relative(this.cf.paths.root(extra), this.path())
        );
    }
    ...
    async minify() {
        if (this.extension() === '.js') {
            const output = await Terser.minify(
                this.read(),
                this.cf.config.terser.terserOptions
            );
        ...
        if (this.extension() === '.css') {
            const output = await new UglifyCss(this.cf.config.cleanCss).minify(
                this.read()
            );
