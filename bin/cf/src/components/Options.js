...
class Options {
    ...
    messages(options) {
        ...
        if ('extractVueStyles' in options) {
            messages.push({
                type: 'warn',
                text:
                    'The option extractVueStyles has been moved. Please pass the extractStyles option to cf.vue() instead.'
            });
        }
        ...
        if ('globalVueStyles' in options) {
            messages.push({
                type: 'warn',
                text:
                    'The option globalVueStyles has been moved. Please pass the globalStyles option to cf.vue() instead.'
            });
        }
    ...
    buildVueExample(options) {
        ...
        return `cf.vue(${JSON.stringify(props, null, 2)}})`;
    }
}
