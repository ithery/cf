...
class Extract {
     ...
     *
     * cf.extract() or cf.extractVendor()
     */
    name() {
    ...
    normalizeExtraction(config = null, output = null) {
        ...
        if (typeof config === 'string') {
            if (output !== null || !config.endsWith('.js')) {
                throw new Error(
                    'cf.extract(string) expects a file path as its only argument'
                );
            }
    ...
    buildLibraryRegex(libraries = []) {
        ...
        if (Array.isArray(libraries)) {
            ...
            throw new Error(
                `Unexpected type [${typeof libraries}] passed to cf.extract({ libraries: … }). ` +
