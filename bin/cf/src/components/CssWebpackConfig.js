...
class CssWebpackConfig extends AutomaticComponent {
    ...
    webpackRules() {
        return [
            ...
            },
            {
                ...
                type: 'less',
                test: /\.less$/,
                loader: { loader: CF.resolve('less-loader') }
            },
            {
                command: 'stylus',
                type: 'stylus',
                test: /\.styl(us)?$/,
                loader: { loader: CF.resolve('stylus-loader') }
            }
    ...
    createLoaderList(rule, useCssModules) {
        return [
            ...
            {
                loader: CF.resolve('postcss-loader'),
                options: {
                    postcssOptions: {
    ...
    excludePathsFor(command) {
        let exclusions = CF.components.get(command);
    ...
    static afterLoaders({ method = 'auto', location = 'default' } = {}) {
        ...
        if (method === 'inline') {
            if (this.wantsVueStyleLoader && location === 'default') {
                loaders.push({ loader: CF.resolve('vue-style-loader') });
            } else {
                loaders.push({ loader: CF.resolve('style-loader') });
            }
    ...
    static get wantsVueStyleLoader() {
        const VueFeature = CF.components.get('vue');
    ...
    static beforeLoaders({ type, injectGlobalStyles }) {
        ...
        if (CF.globalStyles && injectGlobalStyles) {
            let resources =
                CssWebpackConfig.normalizeGlobalStyles(CF.globalStyles)[type] || [];

            if (resources.length) {
                loaders.push({
                    loader: CF.resolve('sass-resources-loader'),
                    options: {
    ...
    static normalizeGlobalStyles(styles) {
        ...
        return mapValues(styles, files => {
            return Array.wrap(files).map(file => CF.paths.root(file));
        });
    }
}
