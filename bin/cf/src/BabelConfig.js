...
class BabelConfig {
     ...
     *
     * @param {BabelOptions} cfBabelConfig
     * @deprecated
     */
    static generate() {
    ...
    generate() {
        return this.mergeAll([
            this.default(),
            this.getCustomConfig(this.cf.config.babelConfig),
            {
                root: this.cf.paths.root(),
                babelrc: true,
                configFile: true,
                babelrcRoots: ['.', this.cf.paths.root()]
            }
        ]);
    }
    ...
    /**
     * Get the babel config setup when using cf.babelConfig()
     *
     * @internal
