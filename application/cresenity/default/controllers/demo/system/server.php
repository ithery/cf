...
class Controller_Demo_System_Server extends \Cresenity\Demo\Controller {
    ...
    public function index() {
        $app = c::app();
        $app->title('System | Server');

        $app->addP()->add('Demo for get server information');

        $traitLink = CElement_Factory::create(CElement_Element_A::class)->setAttr('href', c::url('demo/system/server/trait'))->addClass('text-primary')->add('link');
        $app->addDiv()->addClass('mb-3')->add('for demo server information use trait from Cresenity Framework, use this ')->add($traitLink);
        ...
        $cardData['properties'] = [];
        $cardData['properties'][] = ['label' => 'Total a', 'value' => CHelper::formatter()->formatSize($totalMemory)];
