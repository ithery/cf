...
class Controller_Users extends CController {
    ...
    public function edit($id = '') {
        ...
        $app->title($title);
        $app->addBreadcrumb(clang::__('Users'), curl::base() . 'users');
    ...
    public function detail($user_id, $method = '') {
        ...
        $app->addBreadcrumb(clang::__('User'), curl::base() . 'users');
        $org = $app->org();
        if ($org != null) {
            $login_user = $app->user();
    ...
    public function delete($id = '') {
        ...
        if ($error == 0) {
            try {
                $db->update('users', ['status' => 0, 'updated' => date('Y-m-d H:i:s'), 'updatedby' => $userapp->username], ['user_id' => $id]);
            } catch (Exception $e) {
                $error++;
