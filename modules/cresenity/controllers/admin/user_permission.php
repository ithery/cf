...
class User_permission_Controller extends CController {
    ...
    public static function cell_callback($table,$col,$row,$text) {
		...
		switch($col) {
			...
			case "access":
				...
				if(!isset($row["controller"])||$row["controller"]=="") return "";
				$checked = "";
				$role_id = $row["role_id"];
			...
			case "permission":
				...
				if(isset($row["action"])) {
					...
					foreach($row["action"] as $act) {
						if(!cnav::permission_available($act["name"],$row)) continue;
						$active = "";
	...
	public function save() {
		...
		if ($post!=null) {
			...
			try{
				...
				if($role_id=="PUBLIC") $role_id=null;
	...
	public function index() {
		...
		$org_list = CJDB::instance()->get_list('org','org_id','name');
		...
		$app_data=$cdb->get('domain',array("org_id"=>$org_id))->result_array();
		$app_list = array();
		foreach($app_data as $domain) {
			$c_app_id=$domain["app_id"];
			$app_result = $cdb->get('app',array('app_id'=>$c_app_id));
			$app_name = $app_result[0]->name;
			$app_list[$c_app_id]=$app_name;
		}
		
		//if array is empty, we make default is app with id 1
		if(empty($app_list)) {
			$app_id=1;
			$app_result = $cdb->get('app',array('app_id'=>$app_id));
			$app_name = $app_result[0]->name;
		...
		if(ccfg::get("web_front")&&$app_id=='2') {
		...
		$form->add_field('org-field')->set_label('Org')->add_control('org_id','select')->set_value($org_id)->set_list($org_list);
		$form->add_field('application-field')->set_label('Application')->add_control('app_id','select')->set_value($app_id)->set_list($app_list);
		$form->add_field('role-field')->set_label('Role')->add_control('role_id','select')->set_value($role_id)->set_list($role_list);
		$form->add_field('check-all-field')->set_label('Check All')->add_control('check_all','checkbox')->set_value("1");
		
		if(strlen($role_id)>0) {
			$data = cnav::app_user_rights_array($app_id,$role_id);
			...
			$table->cell_callback_func(array("User_permission_Controller","cell_callback"));
			...
			$app->add('<div class="well">ERROR, NO ROLE</div>');
		}
		
		$app->add_js("
			jQuery(document).ready(function() {
				$('#app_id, #role_id, #org_id').change(function() {
				...
				$('#user_rights_form').submit(function(e) {
					...
					jQuery.ajax({
						type: 'post',
						url: '".curl::base()."admin/user_permission/save',
						dataType: 'json',
						data: {'access':data,'role_id':$('#role_id').val(),'app_id':$('#app_id').val(),'org_id':$('#org_id').val()}
					}).done(function( data ) {
						var type='error';
						if(data.result==1) {
							type='success';
						...
						if(data.result==1) {
							window.location.href='".curl::base()."admin/user_permission?app_id='+$('#app_id').val()+'&role_id='+$('#role_id').val()+'&org_id='+$('#org_id').val();
						}
