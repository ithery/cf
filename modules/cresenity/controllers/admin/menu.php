...
class Menu_Controller extends CController {
	  ...
	  public static function cell_callback($table,$col,$row,$text) {
		...
		switch($col) {
			...
			case "access":
				...
				$checked = "";
				if(cnav::have_access($row,$row["role_id"],$row["app_id"])) {
			...
			case "permission":
				...
				if(isset($row["action"])) {
					...
					foreach($row["action"] as $act) {
						if(!cnavcfg::permission_available($act["name"],$row)) continue;
						$active = "";
	...
	public function save() {
		...
		if ($post!=null) {
			...
			try{
				...
				if($error==0) {
					$db->delete("role_permission",array("role_id"=>$role_id));
					$db->delete("role_nav",array("role_id"=>$role_id));
					$access = $post["access"];
			...
			if($error==0) {
				$db->commit();
				$result["result"]="1";
				$result["message"]="User Rights Successfully Modified !";
	...
	public function index() {
		...
		$data = cnav::as_user_rights_array($app_id,$role_id);
		
		
		$form = $app->add_form('menu_form');
		...
		$table->cell_callback_func(array("Menu_Controller","cell_callback"));
