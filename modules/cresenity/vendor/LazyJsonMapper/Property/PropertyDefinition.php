...
class PropertyDefinition {
     ...
     * @var string[]
     */
    public static $BASIC_TYPES = ['bool', 'int', 'float', 'string'];
    ...
    $definitionStr = null, $baseNamespace = '') {
        ...
        if (is_string($baseNamespace)) { // Custom namespace.
            if (strlen($baseNamespace) > 0 && ($baseNamespace[0] === '\\' || substr($baseNamespace, -1) === '\\')) {
                throw new BadPropertyDefinitionException(sprintf(
                        'Invalid namespace "%s". The namespace is not allowed to start or end with a backslash. Use __NAMESPACE__ format.', $baseNamespace
                ));
            }
        } else {
        ...
        try {
            ...
            if ($fullClassPath !== LazyJsonMapper::class && !$reflector->isSubClassOf('\\' . LazyJsonMapper::class)) {
                throw new BadPropertyDefinitionException(sprintf(
                        'Class "\\%s" must inherit from LazyJsonMapper.', $fullClassPath
                ));
            }
            ...
            throw new BadPropertyDefinitionException(sprintf(
                    'Reflection failed for class "%s". Reason: "%s".', $fullClassPath, $e->getMessage()
            ));
        }
    }
    ...
    public function asString() {
        return sprintf(
                '%s%s%s', $this->isObjectType ? '\\' : '', $this->propType !== null ? $this->propType : 'mixed', str_repeat('[]', $this->arrayDepth)
        );
    }
