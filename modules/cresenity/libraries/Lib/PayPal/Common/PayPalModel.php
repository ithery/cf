 ...
 */
class PayPal_Common_PayPalModel
{
    ...
    public function __construct($data = null)
    {
        switch (gettype($data)) {
            case "NULL":
                break;
            case "string":
                PayPal_Validation_JsonValidator::validate($data);
                $this->fromJson($data);
                break;
            case "array":
                $this->fromArray($data);
    ...
    public static function getList($data)
    {
        ...
        if (PayPal_Validation_JsonValidator::validate($data)) {
            // It is valid JSON
    ...
    private function _convertToArray($param)
    {
        ...
        foreach ($param as $k => $v) {
            if ($v instanceof PayPal_Common_PayPalModel) {
                $ret[$k] = $v->toArray();
        ...
        if (!empty($arr)) {
            ...
            foreach ($arr as $k => $v) {
                ...
                if (is_array($v)) {
                    ...
                    if (($clazz = PayPal_Common_ReflectionUtil::getPropertyClass(get_class($this), $k)) != null){
                        // If the value is an associative array, it means, its an object. Just make recursive call to it.
                        if (empty($v)){
                            if (PayPal_Common_ReflectionUtil::isPropertyClassArray(get_class($this), $k)) {
                        ...
                        } elseif (PayPal_Common_ArrayUtil::isAssocArray($v)) {
                            /** @var self $o */
                            $o = new $clazz();
                            $o->fromArray($v);
                            $this->assignValue($k, $o);
