 ...
 */
class PayPal_Auth_OAuthTokenCredential extends PayPal_Common_PayPalResourceModel
{
     ...
     */
    public static $AUTH_HANDLER = 'PayPal_Handler_OauthHandler';
    ...
    public function __construct($clientId, $clientSecret)
    {
        ...
        $this->cipher = new PayPal_Security_Cipher($this->clientSecret);
    ...
    public function getAccessToken($config)
    {
        ...
        $token = PayPal_Cache_AuthorizationCache::pull($config, $this->clientId);
        if ($token) {
            // We found it
            ...
            if (!array_key_exists('accessTokenEncrypted', $token)) {
                PayPal_Cache_AuthorizationCache::push($config, $this->clientId, $this->encrypt($this->accessToken), $this->tokenCreateTime, $this->tokenExpiresIn);
            } else {
    ...
    {
        $httpConfig = new PayPal_Core_PayPalHttpConfig(null, 'POST', $config);
        ...
        foreach ($handlers as $handler) {
            if (!is_object($handler)) {
                ...
                $fullHandler = (string)$handler;
                $handler = new $fullHandler(new PayPal_Rest_ApiContext($this));
            }
    ...
    private function generateAccessToken($config, $refreshToken = null)
    {
        ...
        if ($response == null || !isset($response["access_token"]) || !isset($response["expires_in"])) {
            ...
            throw new PayPal_Exception_PayPalConnectionException(null, "Could not generate new Access token. Invalid response from server: ");
        } else {
