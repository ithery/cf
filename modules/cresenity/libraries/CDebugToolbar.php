...
class CDebugToolbar {
    ...
    public static function render($print = FALSE) {
        ...
        $config = CF::config('debug_toolbar');
        ...
        $template->set('scripts', file_get_contents(CF::find_file('views', 'toolbar', 'js')));

        // CSS for toolbar
        $styles = file_get_contents(CF::find_file('views', 'toolbar', 'css'));
    ...
    public static function get_configs() {
        ...
        foreach ($inc_paths as $inc_path) {
            foreach ((array) glob($inc_path . '/config/*.php') as $filename) {
                ...
                if (in_array($filename, (array) CF::config('debug_toolbar.skip_configs'))) {
                    continue;
                }

                if (!isset($configs[$filename])) {
                    try {
                        $configs[$filename] = CF::config($filename)->as_array();
                    } catch (Exception $e) {
    ...
    public static function is_enabled() {
        ...
        if (CF::$environment == CF::PRODUCTION) {
            return FALSE;
        }
