...
class CParser_HtmlParser_Tokenizer_Helper {
    ...
    public static function ifElseState($upper, $successState, $failureState) {
        ...
        if ($upper === $lower) {
            return function (CParser_HtmlParser_Tokenizer $t, $c) use ($upper, $lower) {
                if ($c === $lower) {
            ...
            return function (CParser_HtmlParser_Tokenizer $t, $c) use($lower, $upper) {
                if ($c === $lower || $c === $upper) {
                    $t->setState($successState);
                } else {
                    $t->setState($failureState);
    ...
    public static function stateBeforeEntity() {
        static::ifElseState(
                "#", State . BeforeNumericEntity, State . InNamedEntity
        );
    }

    public static function stateBeforeNumericEntity() {
        static::ifElseState(
                "X", State . InHexEntity, State . InNumericEntity
        );
    }
