...
class JmesPath_Parser
{
    ...
    private $tpos;
    private $expression;
    private static $nullToken = ['type' => JmesPath_Lexer::T_EOF];
    private static $currentNode = ['type' => JmesPath_Lexer::T_CURRENT];

    private static $bp = [
        JmesPath_Lexer::T_EOF               => 0,
        JmesPath_Lexer::T_QUOTED_IDENTIFIER => 0,
        JmesPath_Lexer::T_IDENTIFIER        => 0,
        JmesPath_Lexer::T_RBRACKET          => 0,
        JmesPath_Lexer::T_RPAREN            => 0,
        JmesPath_Lexer::T_COMMA             => 0,
        JmesPath_Lexer::T_RBRACE            => 0,
        JmesPath_Lexer::T_NUMBER            => 0,
        JmesPath_Lexer::T_CURRENT           => 0,
        JmesPath_Lexer::T_EXPREF            => 0,
        JmesPath_Lexer::T_COLON             => 0,
        JmesPath_Lexer::T_PIPE              => 1,
        JmesPath_Lexer::T_OR                => 2,
        JmesPath_Lexer::T_AND               => 3,
        JmesPath_Lexer::T_COMPARATOR        => 5,
        JmesPath_Lexer::T_FLATTEN           => 9,
        JmesPath_Lexer::T_STAR              => 20,
        JmesPath_Lexer::T_FILTER            => 21,
        JmesPath_Lexer::T_DOT               => 40,
        JmesPath_Lexer::T_NOT               => 45,
        JmesPath_Lexer::T_LBRACE            => 50,
        JmesPath_Lexer::T_LBRACKET          => 55,
        JmesPath_Lexer::T_LPAREN            => 60,
    ];

    /** @var array Acceptable tokens after a dot token */
    private static $afterDot = [
        JmesPath_Lexer::T_IDENTIFIER        => true, // foo.bar
        JmesPath_Lexer::T_QUOTED_IDENTIFIER => true, // foo."bar"
        JmesPath_Lexer::T_STAR              => true, // foo.*
        JmesPath_Lexer::T_LBRACE            => true, // foo[1]
        JmesPath_Lexer::T_LBRACKET          => true, // foo{a: 0}
        JmesPath_Lexer::T_FILTER            => true, // foo.[?bar==10]
    ];

    /**
     * @param Lexer $lexer Lexer used to tokenize expressions
     */
    public function __construct(JmesPath_Lexer $lexer = null)
    {
        $this->lexer = $lexer ?: new JmesPath_Lexer();
    }
    ...
    public function parse($expression)
    {
        ...
        if ($this->token['type'] === JmesPath_Lexer::T_EOF) {
            return $result;
        }
    ...
    private function nud_quoted_identifier()
    {
        $token = $this->token;
        $this->next();
        $this->assertNotToken(JmesPath_Lexer::T_LPAREN);
        return ['type' => 'field', 'value' => $token['value']];
    }
    ...
    {
        $this->next();
        return ['type' => JmesPath_Lexer::T_EXPREF, 'children' => [$this->expr(self::$bp[JmesPath_Lexer::T_EXPREF])]];
    }

    private function nud_not()
    {
        $this->next();
        return ['type' => JmesPath_Lexer::T_NOT, 'children' => [$this->expr(self::$bp[JmesPath_Lexer::T_NOT])]];
    }

    private function nud_lparen() {
        $this->next();
        $result = $this->expr(0);
        if ($this->token['type'] !== JmesPath_Lexer::T_RPAREN) {
    ...
    {
        static $validKeys = [JmesPath_Lexer::T_QUOTED_IDENTIFIER => true, JmesPath_Lexer::T_IDENTIFIER => true];
        $this->next($validKeys);
        $pairs = [];

        do {
            $pairs[] = $this->parseKeyValuePair();
            if ($this->token['type'] == JmesPath_Lexer::T_COMMA) {
                $this->next($validKeys);
            }
        } while ($this->token['type'] !== JmesPath_Lexer::T_RBRACE);
    ...
    private function nud_lbracket()
    {
        ...
        if ($type == JmesPath_Lexer::T_NUMBER || $type == JmesPath_Lexer::T_COLON) {
            return $this->parseArrayIndexExpression();
        } elseif ($type == JmesPath_Lexer::T_STAR && $this->lookahead() == JmesPath_Lexer::T_RBRACKET) {
    ...
    {
        static $nextTypes = [JmesPath_Lexer::T_NUMBER => true, JmesPath_Lexer::T_COLON => true, JmesPath_Lexer::T_STAR => true];
        $this->next($nextTypes);
        switch ($this->token['type']) {
            case JmesPath_Lexer::T_NUMBER:
            case JmesPath_Lexer::T_COLON:
                return [
                    'type' => 'subexpression',
    ...
    private function led_flatten(array $left)
    {
        ...
        return [
            ...
            'from'     => 'array',
            'children' => [
                ['type' => JmesPath_Lexer::T_FLATTEN, 'children' => [$left]],
                $this->parseProjection(self::$bp[JmesPath_Lexer::T_FLATTEN])
            ]
        ];
    }
    ...
    private function led_dot(array $left)
    {
        ...
        if ($this->token['type'] == JmesPath_Lexer::T_STAR) {
            return $this->parseWildcardObject($left);
        }

        return [
            'type'     => 'subexpression',
            'children' => [$left, $this->parseDot(self::$bp[JmesPath_Lexer::T_DOT])]
        ];
    }
    ...
    private function led_or(array $left)
    {
        $this->next();
        return [
            'type'     => JmesPath_Lexer::T_OR,
            'children' => [$left, $this->expr(self::$bp[JmesPath_Lexer::T_OR])]
        ];
    }
    ...
    private function led_and(array $left)
    {
        $this->next();
        return [
            'type'     => JmesPath_Lexer::T_AND,
            'children' => [$left, $this->expr(self::$bp[JmesPath_Lexer::T_AND])]
        ];
    }
    ...
    private function led_pipe(array $left)
    {
        $this->next();
        return [
            'type'     => JmesPath_Lexer::T_PIPE,
            'children' => [$left, $this->expr(self::$bp[JmesPath_Lexer::T_PIPE])]
        ];
    }
    ...
    private function led_lparen(array $left)
    {
        ...
        while ($this->token['type'] != JmesPath_Lexer::T_RPAREN) {
            $args[] = $this->expr(0);
            if ($this->token['type'] == JmesPath_Lexer::T_COMMA) {
                $this->next();
            }
    ...
    private function led_filter(array $left)
    {
        ...
        if ($this->token['type'] != JmesPath_Lexer::T_RBRACKET) {
            throw $this->syntax('Expected a closing rbracket for the filter');
        }

        $this->next();
        $rhs = $this->parseProjection(self::$bp[JmesPath_Lexer::T_FILTER]);
    ...
    private function led_comparator(array $left)
    {
        ...
        return [
            'type'     => JmesPath_Lexer::T_COMPARATOR,
            'value'    => $token['value'],
            'children' => [$left, $this->expr(self::$bp[JmesPath_Lexer::T_COMPARATOR])]
        ];
    }
    ...
    private function parseProjection($bp)
    {
        ...
        if (self::$bp[$type] < 10) {
            return self::$currentNode;
        } elseif ($type == JmesPath_Lexer::T_DOT) {
            $this->next(self::$afterDot);
            return $this->parseDot($bp);
        } elseif ($type == JmesPath_Lexer::T_LBRACKET || $type == JmesPath_Lexer::T_FILTER) {
            return $this->expr($bp);
        }
    ...
    {
        if ($this->token['type'] == JmesPath_Lexer::T_LBRACKET) {
            $this->next();
            return $this->parseMultiSelectList();
        }
    ...
    {
        static $validColon = [JmesPath_Lexer::T_COLON => true];
    ...
    private function parseWildcardObject(array $left = null)
    {
        ...
        return [
            ...
            'children' => [
                $left ?: self::$currentNode,
                $this->parseProjection(self::$bp[JmesPath_Lexer::T_STAR])
            ]
        ];
    }
    ...
    {
        static $getRbracket = [JmesPath_Lexer::T_RBRACKET => true];
    ...
    {
        static $matchNext = [
            JmesPath_Lexer::T_NUMBER   => true,
            JmesPath_Lexer::T_COLON    => true,
            JmesPath_Lexer::T_RBRACKET => true
        ];
        ...
        do {
            if ($this->token['type'] == JmesPath_Lexer::T_COLON) {
                $pos++;
                $expected = $matchNext;
            } elseif ($this->token['type'] == JmesPath_Lexer::T_NUMBER) {
                $parts[$pos] = $this->token['value'];
                $expected = [JmesPath_Lexer::T_COLON => true, JmesPath_Lexer::T_RBRACKET => true];
            }
            $this->next($expected);
        } while ($this->token['type'] != JmesPath_Lexer::T_RBRACKET);
    ...
    private function parseMultiSelectList()
    {
        ...
        do {
            ...
            if ($this->token['type'] == JmesPath_Lexer::T_COMMA) {
                $this->next();
                $this->assertNotToken(JmesPath_Lexer::T_RBRACKET);
            }
        } while ($this->token['type'] !== JmesPath_Lexer::T_RBRACKET);
        $this->next();
