...
final class JmesPath_Env
{
    ...
    public static function search($expression, $data)
    {
        static $runtime;
        if (!$runtime) {
            $runtime = JmesPath_Env::createRuntime();
        }
    ...
    public static function createRuntime()
    {
        switch ($compileDir = getenv(self::COMPILE_DIR)) {
            case false: return new JmesPath_AstRuntime();
            case 'on': return new JmesPath_CompilerRuntime();
            default: return new JmesPath_CompilerRuntime($compileDir);
        }
    }
