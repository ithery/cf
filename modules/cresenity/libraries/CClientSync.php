...
/**
 * @deprecated dp not use this anymore
 */
...
class CClientSync {
            ...
            if (isset($data_table['after_sync'])) {
                ...
                if (isset($after_sync['delete'])) {
                    if ($after_sync['delete']) {
                        $this->after_sync_delete[] = $table_name;
                    }
                }
            }
        }
    ...
    private function initialize($method) {
        ...
        if ($error == 0) {
            ...
            if (strlen($this->client_last_update) == 0) {
                $this->client_last_update = '2000-01-01 00:00:00';
            }
        }
        if ($error == 0) {
    ...
    private function query_statements($table) {
        ...
        foreach ($r as $row) {
            ...
            $statements[] = [
                'table' => $table,
                'data' => $row,
                'where' => ['org_id' => $this->org_id, 'store_id' => $this->store_id, $table . '_id' => $row[$table . '_id']],
                'method' => 'replace'
            ];
            foreach ($childs_table as $child_table) {
                ...
                foreach ($r2 as $row2) {
                    ...
                    $statements[] = [
                        'table' => $child_table,
                        'data' => $row2,
                        'where' => ['org_id' => $this->org_id, 'store_id' => $this->store_id, $child_table . '_id' => $row2[$child_table . '_id']],
                        'method' => 'replace'
                    ];
                }
            }

            $this->synced_data[] = ['table' => $table, 'id' => $row[$table . '_id']];
        }
