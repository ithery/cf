...
class CExcel_Reader {
    ...
    use CExcel_Trait_DelegatedMacroableTrait,
        CExcel_Trait_HasEventBusTrait;
    ...
    public function setDefaultValueBinder() {
        Cell::setValueBinder(new CExcel_DefaultValueBinder());
        return $this;
    }
    ...
    private function buildSheetImports($import): array {
        ...
        if ($import instanceof WithMultipleSheets) {
            ...
            if (
                    method_exists($this->reader, 'setLoadSheetsOnly') && count(array_filter(array_keys($sheetImports), 'is_numeric')) === 0
            ) {
                $this->reader->setLoadSheetsOnly(array_keys($sheetImports));
            }
        }
    ...
    private function getReader($import, $filePath, string $readerType = null, string $disk = null): IReader {
        ...
        return ReaderFactory::make(
                        $import, $this->currentFile, $readerType
        );
    }
