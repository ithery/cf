 ...
 * @since Jun 25, 2019, 9:16:50 PM
 * @license Ittron Global Teknologi <ittron.co.id>
 */
class CXMLRPC_Server {

    /**
     * Contains a reference to the CXMLRPC documentor object.
     * @see CXMLRPC_Documentor
     */
    ...
    /**
     * Creates a new instance of the CXMLRPC server.
     ...
     * @see CXMLRPC_Server::setOutputOption()
     * @throws CXMLRPC_InvalidArgumentException (ripcord::unknownServiceType) when passed an incorrect service
     * @throws CXMLRPC_ConfigurationException (ripcord::xmlrpcNotInstalled) when the xmlrpc extension in not available.
     */
    function __construct($services = null, $options = null, $documentor = null) {
        if (!function_exists('xmlrpc_server_create')) {
            throw new CXMLRPC_ConfigurationException('PHP XMLRPC library is not installed', ripcord::xmlrpcNotInstalled);
        }
    ...
    public function addService($service, $serviceName = 0) {
        if (is_object($service)) {
            ...
            throw new CXMLRPC_InvalidArgumentException('Unknown service type ' . $serviceName, ripcord::unknownServiceType);
        }
        ...
        if (is_array($methods)) {
            foreach ($methods as $method) {
                if (substr($method->name, 0, 1) != '_' && !$method->isPrivate() && !$method->isProtected()) {
                    $rpcMethodName = $serviceName . $method->name;
                    $this->addMethod(
                            $rpcMethodName, array($service, $method->name)
                    );
                }
            }
        }
        ...
        if (!$request_xml) {
            if (( $query = $_SERVER['QUERY_STRING'] ) && isset($this->wsdl[$query]) && $this->wsdl[$query]) {
                ...
                $result = xmlrpc_encode_request(
                        null, ripcord::fault(-1, 'No request xml found.'), $this->outputOptions
                );
    ...
    private function parseRequest($request_xml) {
        ...
        if (!$xml && !$xml->getNamespaces()) {
            ...
            return xmlrpc_encode_request(
                    null, ripcord::fault(-3, 'Invalid Method Call - CXMLRPC Server accepts only XML-RPC, SimpleRPC or SOAP 1.1 calls'), $this->outputOptions
            );
        } else {
            ...
            if ($methodCall) { //xml-rpc
                ...
                if (!$methodName) {
                    return xmlrpc_encode_request(
                            null, ripcord::fault(-3, 'Invalid Method Call - No methodName given'), $this->outputOptions
                    );
                }
            }
        }
    ...
    private function multiCall($params = null) {
        if ($params && is_array($params)) {
            ...
            $result = xmlrpc_encode_request(
                    null, ripcord::fault(-2, 'Illegal or no params set for system.multiCall'), $this->outputOptions
            );
        }
        return $result;
    }
    ...
    public function handle($request_xml) {
        ...
        if ($method == 'system.multiCall' || $method == 'system.multicall') {
            ...
            try {
                ...
                $result = xmlrpc_encode_request(
                        null, ripcord::fault($e->getCode(), $e->getMessage()), $this->outputOptions
                );
            }
        }
        return $result;
     ...
     * @return mixed
     * @throws CXMLRPC_InvalidArgumentException (ripcord::cannotRecurse) when passed a recursive multiCall
     * @throws CXMLRPC_BadMethodCallException (ripcord::methodNotFound) when the requested method isn't available.
     */
    public function call($method, $args = null) {
        if (isset($this->methods[$method])) {
            ...
            if (substr($method, 0, 7) == 'system.') {
                ...
                throw new CXMLRPC_BadMethodCallException('Method ' . $method . ' not found.', ripcord::methodNotFound);
            }
        }
    }
