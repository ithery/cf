 ...
 */
class GuzzleHttp_Ring_Client_CurlFactory {
    ...
    callable $handler, array $request, array $response, array $headers, $body
        ...
        if (!empty($headers)) {
            ...
            $headerList = GuzzleHttp_Ring_Core::headersFromLines($headers);
            ...
            $response['body'] = $body;
            GuzzleHttp_Ring_Core::rewindBody($response);
        }
    ...
    callable $handler, array $request, array $response
        ...
        $error = isset($response['curl']['errno']) && isset($connectionErrors[$response['curl']['errno']]) ? new GuzzleHttp_Ring_Exception_ConnectException($message) : new GuzzleHttp_Ring_Exception_RingException($message);
    ...
    private function getDefaultOptions(array $request, array &$headers) {
        $url = GuzzleHttp_Ring_Core::url($request);
        ...
        $options = [
            ...
            private function applyMethod(array $request, array &$options) {
                ...
                switch ($request['http_method']) {
                    ...
                    case 'POST':
                        ...
                        if (!GuzzleHttp_Ring_Core::hasHeader($request, 'Content-Length')) {
            ...
            private function applyBody(array $request, array &$options) {
                $contentLength = GuzzleHttp_Ring_Core::firstHeader($request, 'Content-Length');
                ...
                if (($size !== null && $size < 1000000) ||
                    ...
                    $options[CURLOPT_POSTFIELDS] = GuzzleHttp_Ring_Core::body($request);
                ...
                if (!GuzzleHttp_Ring_Core::hasHeader($request, 'Expect')) {
                    $options[CURLOPT_HTTPHEADER][] = 'Expect:';
                }

                // cURL sometimes adds a content-type by default. Prevent this.
                if (!GuzzleHttp_Ring_Core::hasHeader($request, 'Content-Type')) {
            ...
            private function applyHeaders(array $request, array &$options) {
                ...
                if (!GuzzleHttp_Ring_Core::hasHeader($request, 'Accept')) {
                ...
                foreach ($request['client'] as $key => $value) {
                    switch ($key) {
                        ...
                        case 'decode_content':
                            ...
                            $accept = GuzzleHttp_Ring_Core::firstHeader($request, 'Accept-Encoding');
                            if ($accept) {
                            ...
                            if (is_string($value)) {
                                if (!is_dir(dirname($value))) {
                                    throw new \RuntimeException(sprintf(
                                            'Directory %s does not exist for save_to value of %s', dirname($value), $value
                                    ));
                                }
                        ...
                        case 'debug':
                            ...
                            if ($value) {
                                $options[CURLOPT_STDERR] = GuzzleHttp_Ring_Core::getDebugResource($value);
            ...
            callable $handler, array $request, array $response
                ...
                if (!GuzzleHttp_Ring_Core::rewindBody($request)) {
