...
class InstagramAPI_Client {
    ...
    /**
     * @var InstagramAPI_ClientMiddleware
     */
    private $_clientMiddleware;

    /**
     * @var GuzzleHttp_Cookie_CookieJar
     */
    private $_cookieJar;
     ...
     * Constructor.
     *
     * @param InstagramAPI_Instagram $parent
     */
    public function __construct($parent) {
        ...
        $this->_mapper = new InstagramAPI_JsonMapper();
     ...
     */
    public function updateFromCurrentSettings($resetCookieJar = false) {
    ...
    public function loadCookieJar($resetCookieJar = false) {
        ...
        if ($userCookies['format'] == 'cookiefile') {
            ...
            if (!InstagramAPI_Utils::createFolder($cookieDir)) {
                throw new SettingsException(sprintf(
                        'The "%s" cookie folder is not writable.', $cookieDir
                ));
            }
        ...
        $this->_cookieJar = new GuzzleHttp_Cookie_CookieJar(false, $restoredCookies);
     ...
     *
     * @return GuzzleHttp_Cookie_SetCookie|null A cookie if found, otherwise NULL.
     */
    public function getCookie($name, $domain = null, $path = null) {
        $foundCookie = null;
        if ($this->_cookieJar instanceof GuzzleHttp_Cookie_CookieJar) {
            foreach ($this->_cookieJar->getIterator() as $cookie) {
                if ($cookie->getName() == $name && ($domain === null || $cookie->getDomain() == $domain) && ($path === null || $cookie->getPath() == $path)) {
                    $foundCookie = $cookie;
                    break;
                }
            }
    ...
    public function getCookieJarAsJSON() {
        ...
        $jsonStr = guzzlehttp_json_encode($cookies);
    ...
    public function saveCookieJar() {
        ...
        if ($this->_settingsCookieFormat != 'cookiefile') {
            ...
            if (!empty($this->_settingsCookieFilePath)) {
                ...
                $written = InstagramAPI_Utils::atomicWrite($this->_settingsCookieFilePath, $newCookies);
                if ($written === false) {
                    throw new InstagramAPI_Exception_SettingsException(sprintf(
                            'The "%s" cookie file is not writable.', $this->_settingsCookieFilePath
                    ));
                }
            }
        }
    ...
    protected function _printDebug($method, $url, $uploadedBody, $uploadedBytes, $response, $responseBody) {
        ...
        InstagramAPI_Debug::printRequest($method, $url);
        ...
        if (is_string($uploadedBody)) {
            InstagramAPI_Debug::printPostData($uploadedBody);
        }
        ...
        if ($uploadedBytes !== null) {
            InstagramAPI_Debug::printUpload(InstagramAPI_Utils::formatBytes($uploadedBytes));
        }

        // Display the number of bytes received from the response, and status code.
        if ($response->hasHeader('x-encoded-content-length')) {
            $bytes = InstagramAPI_Utils::formatBytes($response->getHeader('x-encoded-content-length')[0]);
        } else {
            $bytes = InstagramAPI_Utils::formatBytes($response->getHeader('Content-Length')[0]);
        }
        InstagramAPI_Debug::printHttpCode($response->getStatusCode(), $bytes);

        // Display the actual API response body.
        InstagramAPI_Debug::printResponse($responseBody, $this->_parent->truncatedDebug);
    }

    /**
     * Converts a server response to a specific kind of result object.
     *
     * @param InstagramAPI_ResponseInterface        $baseClass      An instance of a class object whose properties to fill with the response.
     *                                              
     * @param mixed                                 $serverResponse A decoded JSON response from Instagram's server.
     *                                             
     * @param Psr_Http_Message_ResponseInterface    $httpResponse   HTTP response object.
     *
     * @throws InstagramAPI_Exception_InstagramException In case of invalid or failed API response.
     *                                                    
     *
     * @return ResponseInterface
     */
    public function getMappedResponseObject(InstagramAPI_ResponseInterface $baseClass, $serverResponse, Psr_Http_Message_ResponseInterface $httpResponse) {
        ...
        if (!is_object($serverResponse)) {
            ...
            switch ($httpStatusCode) {
                case 400:
                    throw new InstagramAPI_Exception_BadRequestException('Invalid request options.');
                case 404:
                    throw new InstagramAPI_Exception_NotFoundException('Requested resource does not exist.');
                default:
                    throw new InstagramAPI_Exception_EmptyResponseException('No response from server. Either a connection or configuration error.');
            }
        }
        ...
        /** @var InstagramAPI_ResponseInterface $responseObject */
        ...
        if (!$responseObject->isOk()) {
            if ($responseObject instanceof InstagramAPI_Response_DirectSendItemResponse && $responseObject->getPayload() !== null) {
                $message = $responseObject->getPayload()->getMessage();
            } else {
                $message = $responseObject->getMessage();
            }
            InstagramAPI_Exception_ServerMessageThrower::autoThrow(
                    get_class($baseClass), $message, $responseObject, $httpResponse
            );
        }
    ...
    array $guzzleOptions = []) {
        $criticalOptions = [
            'cookies' => ($this->_cookieJar instanceof GuzzleHttp_Cookie_CookieJar ? $this->_cookieJar : false),
     ...
     */
    protected function _guzzleRequest(Psr_Http_Message_RequestInterface $request, array $guzzleOptions = []) {
        ...
        try {
            ...
            throw new InstagramAPI_Exception_NetworkException($e);
        }
        ...
        switch ($httpCode) {
            case 429: // "429 Too Many Requests"
                throw new InstagramAPI_Exception_ThrottledException('Throttled by Instagram because of too many API requests.');
                break;
     ...
     */
    protected function _apiRequest(Psr_Http_Message_RequestInterface $request, array $guzzleOptions = [], array $libraryOptions = []) {
        ...
        if ($this->_parent->debug && (!isset($libraryOptions['noDebug']) || !$libraryOptions['noDebug'])) {
            ...
            $this->_printDebug(
                    $request->getMethod(), (string) $request->getUri(), $uploadedBody, $uploadedBytes, $guzzleResponse, (string) $guzzleResponse->getBody());
        }
     ...
     * @return HttpResponseInterface
     */
    public function api(Psr_Http_Message_RequestInterface $request, array $guzzleOptions = []) {
        // Set up headers that are required for every request.
        $request = guzzlehttp_psr7_modify_request($request, [
            'set_headers' => [
                ...
                'X-FB-HTTP-Engine' => InstagramAPI_Constants::X_FB_HTTP_Engine,
                'Accept' => '*/*',
                'Accept-Encoding' => InstagramAPI_Constants::ACCEPT_ENCODING,
                'Accept-Language' => InstagramAPI_Constants::ACCEPT_LANGUAGE,
            ],
        ]);

        // Check the Content-Type header for debugging.
        $contentType = $request->getHeader('Content-Type');
        $isFormData = count($contentType) && reset($contentType) === InstagramAPI_Constants::CONTENT_TYPE;
     ...
     *
     * @return InstagramAPI_ClientMiddleware
     */
