...
class InstagramAPI_Instagram {
     ...
     *
     * @var InstagramAPI_Settings_StorageHandler|null
     */
    public $settings;
    ...
    /** @var InstagramAPI_Request_Account Collection of Account related functions. */
    public $account;

    /** @var InstagramAPI_Request_Business Collection of Business related functions. */
    public $business;

    /** @var InstagramAPI_Request_Collection Collection of Collections related functions. */
    public $collection;

    /** @var InstagramAPI_Request_Creative Collection of Creative related functions. */
    public $creative;

    /** @var InstagramAPI_Request_Direct Collection of Direct related functions. */
    public $direct;

    /** @var InstagramAPI_Request_Discover Collection of Discover related functions. */
    public $discover;

    /** @var InstagramAPI_Request_Hashtag Collection of Hashtag related functions. */
    public $hashtag;

    /** @var InstagramAPI_Request_Internal Collection of Internal (non-public) functions. */
    public $internal;

    /** @var InstagramAPI_Request_Live Collection of Live related functions. */
    public $live;

    /** @var InstagramAPI_Request_Location Collection of Location related functions. */
    public $location;

    /** @var InstagramAPI_Request_Media Collection of Media related functions. */
    public $media;

    /** @var InstagramAPI_Request_People Collection of People related functions. */
    public $people;

    /** @var InstagramAPI_Request_Push Collection of Push related functions. */
    public $push;

    /** @var InstagramAPI_Request_Story Collection of Story related functions. */
    public $story;

    /** @var InstagramAPI_Request_Timeline Collection of Timeline related functions. */
    public $timeline;

    /** @var InstagramAPI_Request_Usertag Collection of Usertag related functions. */
    public $usertag;
     ...
     */
    public function __construct($debug = false, $truncatedDebug = false, array $storageConfig = []) {
        ...
        if (PHP_VERSION_ID < 50600) {
            //throw new InstagramAPI_Exception_InternalException('You must have PHP 5.6 or higher to use the Instagram API library.');
        }
        ...
        $this->account = new InstagramAPI_Request_Account($this);
        $this->business = new InstagramAPI_Request_Business($this);
        $this->collection = new InstagramAPI_Request_Collection($this);
        $this->creative = new InstagramAPI_Request_Creative($this);
        $this->direct = new InstagramAPI_Request_Direct($this);
        $this->discover = new InstagramAPI_Request_Discover($this);
        $this->hashtag = new InstagramAPI_Request_Hashtag($this);
        $this->internal = new InstagramAPI_Request_Internal($this);
        $this->live = new InstagramAPI_Request_Live($this);
        $this->location = new InstagramAPI_Request_Location($this);
        $this->media = new InstagramAPI_Request_Media($this);
        $this->people = new InstagramAPI_Request_People($this);
        $this->push = new InstagramAPI_Request_Push($this);
        $this->story = new InstagramAPI_Request_Story($this);
        $this->timeline = new InstagramAPI_Request_Timeline($this);
        $this->usertag = new InstagramAPI_Request_Usertag($this);

        // Configure the settings storage and network client.
        $self = $this;
        $this->settings = InstagramAPI_Settings_Factory::createHandler(
                        $storageConfig, [
                    ...
                    'onCloseUser' => function ($storage) use ($self) {
                        if ($self->client instanceof InstagramAPI_Client) {
                            $self->client->saveCookieJar();
                        }
                    },
                        ]
        );
        $this->client = new InstagramAPI_Client($this);
        $this->experiments = [];
    }
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_LoginResponse|null A login response if a full (re-)login happens,
     *                                                   otherwise NULL if an existing session is resumed.
     */
    public function login($username, $password, $forceLogin = false, $appRefreshInterval = 1800) {
        ...
        if (!$this->isLoggedIn || $forceLogin) {
            ...
            try {
                $response = $this->request('accounts/login/')
                        ...
                        ->getResponse(new InstagramAPI_Response_LoginResponse());
            } catch (InstagramAPI_Exception_InstagramException $e) {
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_LoginResponse
     */
    public function finishTwoFactorLogin(
    $verificationCode, $twoFactorIdentifier, $appRefreshInterval = 1800) {
        if (empty($this->username) || empty($this->password)) {
            throw new InstagramAPI_Exception_LoginRequiredException(
     ...
     * @throws InstagramAPI_Exception_InstagramException
     */
    protected function _setUser($username, $password) {
        if (empty($username) || empty($password)) {
        ...
        $this->device = new InstagramAPI_Devices_Device(InstagramAPI_Constants::IG_VERSION, InstagramAPI_Constants::VERSION_CODE, InstagramAPI_Constants::USER_AGENT_LOCALE, $savedDeviceString);
        ...
        if ($deviceString !== $savedDeviceString // Brand new device, or missing
            ...
            $this->settings->set('device_id', InstagramAPI_Signatures::generateDeviceId());
            $this->settings->set('phone_id', InstagramAPI_Signatures::generateUUID(true));
            $this->settings->set('uuid', InstagramAPI_Signatures::generateUUID(true));
        ...
        if (empty($this->settings->get('advertising_id'))) {
            $this->settings->set('advertising_id', InstagramAPI_Signatures::generateUUID(true));
        }
        if (empty($this->settings->get('session_id'))) {
            $this->settings->set('session_id', InstagramAPI_Signatures::generateUUID(true));
        }
     ...
     *
     * @param InstagramAPI_Response_LoginResponse $response The login response.
     *
     * @throws \InvalidArgumentException
     * @throws InstagramAPI_Exception_InstagramException
     */
    protected function _updateLoginState(InstagramAPI_Response_LoginResponse $response) {
        ...
        if (!$response instanceof InstagramAPI_Response_LoginResponse || !$response->isOk()) {
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_LoginResponse|null A login response if a full (re-)login is needed
     *                                                   during the login flow attempt, otherwise NULL.
     */
    protected function _sendLoginFlow($justLoggedIn, $appRefreshInterval = 1800) {
        ...
        if ($justLoggedIn) {
            ...
            } catch (InstagramAPI_Exception_LoginRequiredException $e) {
            ...
            if ($lastLoginTime === null || (time() - $lastLoginTime) > $appRefreshInterval) {
                ...
                $this->session_id = InstagramAPI_Signatures::generateUUID(true);
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_LogoutResponse
     *
     * @see Instagram::login()
     */
    public function logout() {
        $response = $this->request('accounts/logout/')
                ...
                ->getResponse(new InstagramAPI_Response_LogoutResponse());
     ...
     * @return bool
     */
    public function isExperimentEnabled($experiment, $param) {
    ...
    public function request($url) {
        return new InstagramAPI_Request($this, $url);
    }
