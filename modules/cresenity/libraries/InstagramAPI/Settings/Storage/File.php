 ...
 */
class InstagramAPI_Settings_Storage_File implements InstagramAPI_Settings_StorageInterface {
    ...
    public function openLocation(array $locationConfig) {
        ...
        $this->_baseFolder = ((isset($locationConfig['basefolder']) && !empty($locationConfig['basefolder'])) ? $locationConfig['basefolder'] : InstagramAPI_Constants::SRC_DIR . '/../sessions/');
    ...
    $oldUsername, $newUsername) {
        ...
        if (!is_dir($oldUser['userFolder'])) {
            throw new InstagramAPI_Exception_SettingsException(sprintf('Cannot move non-existent user folder "%s".', $oldUser['userFolder']));
        }
        if (is_dir($newUser['userFolder'])) {
            throw new InstagramAPI_Exception_SettingsException(sprintf('Refusing to overwrite existing user folder "%s".', $newUser['userFolder']));
        }
        ...
        if (is_file($oldUser['settingsFile']) && !@rename($oldUser['settingsFile'], $newUser['settingsFile'])) {
            throw new InstagramAPI_Exception_SettingsException(sprintf('Failed to move "%s" to "%s".', $oldUser['settingsFile'], $newUser['settingsFile']));
        }
        if (is_file($oldUser['cookiesFile']) && !@rename($oldUser['cookiesFile'], $newUser['cookiesFile'])) {
            throw new InstagramAPI_Exception_SettingsException(sprintf('Failed to move "%s" to "%s".', $oldUser['cookiesFile'], $newUser['cookiesFile']));
        }
    ...
    public function deleteUser($username) {
        ...
        InstagramAPI_Utils::deleteTree($delUser['userFolder']);
    }
    ...
    public function loadUserSettings() {
        ...
        if ($rawData === false) {
            throw new InstagramAPI_Exception_SettingsException(sprintf('Unable to read from settings file "%s".', $this->_settingsFile));
        }
    ...
    array $userSettings, $triggerKey) {
        ...
        InstagramAPI_Utils::atomicWrite($this->_settingsFile, $encodedData);
    }
            ...
            case 2:
                ...
                if (!is_array($loadedSettings)) {
                    throw new SettingsException(sprintf(
                            'Failed to decode corrupt settings file for account "%s".', $this->_username
                    ));
                }
                break;
            default:
                throw new SettingsException(sprintf(
                        'Invalid file settings storage format version "%d".', $dataVersion
                ));
        }
     ...
     */
    private function _generateUserPaths($username) {
    ...
    $folder) {
        if (!InstagramAPI_Utils::createFolder($folder)) {
            throw new SettingsException(sprintf(
                    'The "%s" folder is not writable.', $folder
            ));
        }
    }
