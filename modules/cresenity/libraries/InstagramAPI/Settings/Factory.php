 ...
 */
class InstagramAPI_Settings_Factory {
     ...
     */
    public static function createHandler(array $storageConfig, array $callbacks = []) {
        ...
        switch ($storageConfig['storage']) {
            case 'file':
                ...
                $storageInstance = new InstagramAPI_Settings_Storage_File();
                break;
            case 'mysql':
                // Look for allowed command-line values related to this backend.
                $cmdOptions = self::getCmdOptions([
                ...
                $storageInstance = new InstagramAPI_Settings_Storage_MySQL();
                break;
            case 'sqlite':
                // Look for allowed command-line values related to this backend.
                $cmdOptions = self::getCmdOptions([
                ...
                $storageInstance = new InstagramAPI_Settings_Storage_SQLite();
                break;
            case 'memcached':
                ...
                if (isset($storageConfig['memcached'])) {
                    ...
                    $locationConfig = [
                        ...
                        'persistent_id' => (isset($storageConfig['persistent_id']) ? $storageConfig['persistent_id'] : null),
                        // Array which will be passed to \Memcached::setOptions().
                        'memcached_options' => (isset($storageConfig['memcached_options']) ? $storageConfig['memcached_options'] : null),
                        // Array which will be passed to \Memcached::addServers().
                        // NOTE: Can contain one or multiple servers.
                        'servers' => (isset($storageConfig['servers']) ? $storageConfig['servers'] : null),
                        ...
                        'sasl_username' => (isset($storageConfig['sasl_username']) ? $storageConfig['sasl_username'] : null),
                        'sasl_password' => (isset($storageConfig['sasl_password']) ? $storageConfig['sasl_password'] : null),
                    ];
                }

                $storageInstance = new InstagramAPI_Settings_Storage_Memcached();
                break;
            case 'custom':
                ...
                if (!isset($storageConfig['class']) || !$storageConfig['class'] instanceof InstagramAPI_Settings_StorageInterface) {
            ...
            default:
                throw new InstagramAPI_Exception_SettingsException(sprintf(
                        'Unknown settings storage type "%s".', $storageConfig['storage']
                ));
        }

        // Create the storage handler and connect to the storage location.
        return new InstagramAPI_Settings_StorageHandler(
                $storageInstance, $locationConfig, $callbacks
        );
    }
     ...
     * @return array
     */
    public static function getCmdOptions(array $longOpts) {
     ...
     */
    public static function getUserConfig($settingName, array $storageConfig, array $cmdOptions) {
