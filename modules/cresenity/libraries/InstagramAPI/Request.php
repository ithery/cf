...
require_once dirname(__FILE__) . "/../GuzzleHttp/Psr7/functions.php";

/**
 * Bridge between Instagram Client calls, the object mapper & response objects.
 */
class InstagramAPI_Request {
     ...
     * @param string    $url
     */
    public function __construct(InstagramAPI_Instagram $parent, $url) {
        $this->_parent = $parent;
        $this->_url = $url;
    ...
    protected function _addDefaultHeaders() {
        if ($this->_defaultHeaders) {
            $this->_headers['X-IG-Capabilities'] = InstagramAPI_Constants::X_IG_Capabilities;
            $this->_headers['X-IG-Connection-Type'] = InstagramAPI_Constants::X_IG_Connection_Type;
     ...
     * @return self
     */
    public function setBody(Psr_Http_Message_StreamInterface $stream) {
        $this->_body = $stream;
     ...
     * @return self
     */
    public function setIsMultiResponse($flag = false) {
     ...
     */
    protected function _getStreamForFile(array $file) {
    ...
    protected function _getMultipartBody() {
        ...
        $index = InstagramAPI_Utils::reorderByHashCode(array_merge($this->_posts, $this->_files));
        ...
        return new MultipartStream($elements, InstagramAPI_Utils::generateMultipartBoundary());
    }
    ...
    protected function _closeHandles() {
        ...
        foreach ($this->_handles as $handle) {
            InstagramAPI_Utils::safe_fclose($handle);
        }
        $this->_resetHandles();
    }
    ...
    protected function _getUrlencodedBody() {
        $this->_headers['Content-Type'] = InstagramAPI_Constants::CONTENT_TYPE;

        return guzzlehttp_psr7_stream_for(http_build_query(InstagramAPI_Utils::reorderByHashCode($this->_posts)));
    }

    /**
     * Convert the request's data into its HTTP POST body contents.
     *
     * @return Psr_Http_Message_StreamInterface|null The body stream if POST request; otherwise NULL if GET request.
     */
    protected function _getRequestBody() {
        ...
        if ($this->_signedPost) {
            $this->_posts = InstagramAPI_Signatures::signData($this->_posts, $this->_excludeSigned);
        }
    ...
    protected function _buildHttpRequest() {
        ...
        if (strncmp($endpoint, 'http:', 5) !== 0 && strncmp($endpoint, 'https:', 6) !== 0) {
            $endpoint = InstagramAPI_Constants::$API_URLS[$this->_apiVersion] . $endpoint;
        }
        // Generate the final endpoint URL, by adding any custom query params.
        if (count($this->_params)) {
            $endpoint = $endpoint
                    . (strpos($endpoint, '?') === false ? '?' : '&')
                    . http_build_query(InstagramAPI_Utils::reorderByHashCode($this->_params));
        }
        // Add default headers.
        ...
        // Build HTTP request object.
        return new GuzzleHttp_Psr7_Request($method, $endpoint, $this->_headers, $postData);
    }
     ...
     *
     * @throws InstagramAPI_Exception_LoginRequiredException
     */
    protected function _throwIfNotLoggedIn() {
        ...
        if (!$this->_parent->isLoggedIn) {
            throw new InstagramAPI_Exception_LoginRequiredException('User not logged in. Please call login() and then try again.');
        }
    }
    ...
    $assoc = false) {
        ...
        return InstagramAPI_Client::api_body_decode((string) $body, $assoc);
    }
     ...
     */
    public function getResponse(InstagramAPI_ResponseInterface $baseClass) {
