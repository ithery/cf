...
class InstagramAPI_Signatures {
    ...
    public static function generateSignature($data) {
        return hash_hmac('sha256', $data, InstagramAPI_Constants::IG_SIG_KEY);
    }
     ...
     * @return string
     */
    public static function generateSignatureForPost($data) {
        return 'ig_sig_key_version=' . InstagramAPI_Constants::SIG_KEY_VERSION . '&signed_body=' . self::generateSignature($data) . '.' . urlencode($data);
    }
     ...
     * @return array
     */
    public static function signData(array $data, array $exclude = []) {
        $result = [];
        ...
        $data = json_encode(InstagramAPI_Utils::reorderByHashCode($data));
        // Sign data.
        $result['ig_sig_key_version'] = InstagramAPI_Constants::SIG_KEY_VERSION;
        $result['signed_body'] = self::generateSignature($data) . '.' . $data;
        // Return value must be reordered.
        return InstagramAPI_Utils::reorderByHashCode($result);
    }
