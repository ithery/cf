...
class InstagramAPI_Utils {
     ...
     */
    public static function buildMediaLocationJSON($location) {
    ...
    $photoFilename) {
        ...
        if ($filesize < 1) {
            throw new \InvalidArgumentException(sprintf(
                    'The photo file "%s" is empty.', $photoFilename
            ));
        }
    ...
    $videoFilename) {
        ...
        if ($filesize < 1) {
            throw new \InvalidArgumentException(sprintf(
                    'The video "%s" is empty.', $videoFilename
            ));
        }
    ...
    $targetFeed, MediaDetails $mediaDetails) {
        ...
        if ($mediaDetails instanceof PhotoDetails) {
            ...
            if ($width < PhotoResizer::MIN_WIDTH || $width > PhotoResizer::MAX_WIDTH) {
                throw new \InvalidArgumentException(sprintf(
                        'Instagram only accepts photos that are between %d and %d pixels wide. Your file "%s" is %d pixels wide.', PhotoResizer::MIN_WIDTH, PhotoResizer::MAX_WIDTH, $mediaFilename, $width
                ));
            }
            ...
            if ($width < VideoResizer::MIN_WIDTH || $width > VideoResizer::MAX_WIDTH) {
                throw new \InvalidArgumentException(sprintf(
                        'Instagram only accepts videos that are between %d and %d pixels wide. Your file "%s" is %d pixels wide.', VideoResizer::MIN_WIDTH, VideoResizer::MAX_WIDTH, $mediaFilename, $width
                ));
            }
        }
        ...
        switch ($targetFeed) {
            ...
            case Constants::FEED_DIRECT_STORY:
                if ($aspectRatio < MediaAutoResizer::MIN_STORY_RATIO || $aspectRatio > MediaAutoResizer::MAX_STORY_RATIO) {
                    throw new \InvalidArgumentException(sprintf(
                            'Instagram only accepts story media with aspect ratios between %.3f and %.3f. Your file "%s" has a %.4f aspect ratio.', MediaAutoResizer::MIN_STORY_RATIO, MediaAutoResizer::MAX_STORY_RATIO, $mediaFilename, $aspectRatio
                    ));
                }
                break;
            default:
                if ($aspectRatio < MediaAutoResizer::MIN_RATIO || $aspectRatio > MediaAutoResizer::MAX_RATIO) {
                    throw new \InvalidArgumentException(sprintf(
                            'Instagram only accepts media with aspect ratios between %.3f and %.3f. Your file "%s" has a %.4f aspect ratio.', MediaAutoResizer::MIN_RATIO, MediaAutoResizer::MAX_RATIO, $mediaFilename, $aspectRatio
                    ));
                }
        }
    }
    ...
    $targetFeed, VideoDetails $videoDetails) {
        ...
        switch ($targetFeed) {
            case Constants::FEED_STORY:
                ...
                if ($duration < 3 || $duration > 15) {
                    throw new \InvalidArgumentException(sprintf(
                            'Instagram only accepts story videos that are between 3 and 15 seconds long. Your story video "%s" is %.3f seconds long.', $videoFilename, $duration
                    ));
                }
                break;
            ...
            case Constants::FEED_DIRECT_STORY:
                ...
                if ($duration < 0.1 || $duration > 15) {
                    throw new \InvalidArgumentException(sprintf(
                            'Instagram only accepts direct videos that are between 0.1 and 15 seconds long. Your direct video "%s" is %.3f seconds long.', $videoFilename, $duration
                    ));
                }
                break;
            default: // timeline
                ...
                if ($duration < 3 || $duration > 60) {
                    throw new \InvalidArgumentException(sprintf(
                            'Instagram only accepts videos that are between 3 and 60 seconds long. Your video "%s" is %.3f seconds long.', $videoFilename, $duration
                    ));
                }
        }
    ...
    $targetFeed, PhotoDetails $photoDetails) {
     ...
     */
    public static function createVideoIcon($targetFeed, $videoFilename) {
        ...
        $tmpPath = self::$defaultTmpPath !== null ? self::$defaultTmpPath : sys_get_temp_dir();
        ...
        try {
            ...
            $resizer = new InstagramAPI_MediaAutoResizer($tmpFilename, ['targetFeed' => $targetFeed]);
    ...
    array $usertags) {
        ...
        foreach ($usertags as $k => $v) {
            if ($k === 'in' || $k === 'removed') {
                ...
                if (!is_array($v) || count($v) != 2 || !isset($v['position']) || !is_array($v['position']) || count($v['position']) != 2 || !isset($v['position'][0]) || !isset($v['position'][1]) || (!is_int($v['position'][0]) && !is_float($v['position'][0])) || (!is_int($v['position'][1]) && !is_float($v['position'][1])) || $v['position'][0] < 0.0 || $v['position'][0] > 1.0 || $v['position'][1] < 0.0 || $v['position'][1] > 1.0 || !isset($v['user_id']) || !is_scalar($v['user_id']) || (!ctype_digit($v['user_id']) && (!is_int($v['user_id']) || $v['user_id'] < 0))) {
        ...
        foreach ($hashtags as $hashtag) {
            ...
            foreach ($hashtag as $k => $v) {
                ...
                // NOTE: Yes, the width/height are relative (0.0-1.0) floats exactly like x/y!
                        (($k === 'x' || $k === 'y' || $k === 'width' || $k === 'height') && (!is_float($v) || $v < 0.0 || $v > 1.0)) || (($k === 'is_sticker' || $k === 'use_custom_title') && !is_bool($v))
                ) {
     ...
     */
    public static function checkMediaType($mediaType) {
    ...
    public static function createFolder($folder) {
        ...
        if ((is_dir($folder) && is_writable($folder)) || (!is_dir($folder) && mkdir($folder, 0755, true)) || chmod($folder, 0755)) {
            return true;
        } else {
            return false;
        }
    ...
    $folder, $keepRootFolder = false) {
        ...
        $files = new \RecursiveIteratorIterator(
                new \RecursiveDirectoryIterator($folder, \RecursiveDirectoryIterator::SKIP_DOTS), \RecursiveIteratorIterator::CHILD_FIRST
        );
    ...
    LoopInterface $loop, array $secureContext = [], $proxyAddress = null) {
