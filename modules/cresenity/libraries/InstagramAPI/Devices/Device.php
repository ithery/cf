 ...
 */
class InstagramAPI_Devices_Device implements InstagramAPI_Devices_DeviceInterface {
    ...
    $appVersion, $versionCode, $userLocale, $deviceString = null, $autoFallback = true) {
        $this->_appVersion = $appVersion;
        $this->_versionCode = $versionCode;
        $this->_userLocale = $userLocale;

        // Use the provided device if a valid good device. Otherwise use random.
        if ($autoFallback && (!is_string($deviceString) || !InstagramAPI_Devices_GoodDevices::isGoodDevice($deviceString))) {
            $deviceString = InstagramAPI_Devices_GoodDevices::getRandomGoodDevice();
        }
    ...
    protected function _initFromDeviceString($deviceString) {
        ...
        $this->_userAgent = InstagramAPI_Devices_UserAgent::buildUserAgent($this->_appVersion, $this->_userLocale, $this);
    ...
    $appName) {
        if (!isset($this->_fbUserAgents[$appName])) {
            $this->_fbUserAgents[$appName] = UserAgent::buildFbUserAgent(
                            $appName, $this->_appVersion, $this->_versionCode, $this->_userLocale, $this
            );
        }
