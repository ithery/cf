 ...
 */
class InstagramAPI_Request_Timeline extends InstagramAPI_Request_RequestCollection {
    ...
    $photoFilename, array $externalMetadata = []) {
    ...
    array $media, array $externalMetadata = []) {
        if (empty($media)) {
            throw new \InvalidArgumentException("List of media to upload can't be empty.");
        }
        if (count($media) < 2 || count($media) > 10) {
            throw new \InvalidArgumentException(sprintf(
                    'Instagram requires that albums contain 2-10 items. You tried to submit %d.', count($media)
            ));
        }
        ...
        foreach ($media as $key => $item) {
            if (!isset($item['file']) || !isset($item['type'])) {
                throw new \InvalidArgumentException(sprintf(
                        'Media at index "%s" does not have the required "file" and "type" keys.', $key
                ));
            }
        ...
        $configure = $this->ig->internal->configureWithRetries(
                'album', function () use ($media, $albumInternalMetadata, $externalMetadata) {
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_TimelineFeedResponse
     */
    public function getTimelineFeed(
    $maxId = null, array $options = null) {
        ...
        return $request->getResponse(new InstagramAPI_Response_TimelineFeedResponse());
    }
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_UserFeedResponse
     */
    public function getUserFeed($userId, $maxId = null, $minTimestamp = null) {
        return $this->ig->request("feed/user/{$userId}/")
                        ...
                        ->getResponse(new InstagramAPI_Response_UserFeedResponse());
    }
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_ArchiveMediaResponse
     */
    public function archiveMedia(
    $mediaId, $mediaType = 'PHOTO', $onlyMe = true) {
        ...
        return $this->ig->request("media/{$mediaId}/{$endpoint}/")
                        ...
                        ->getResponse(new InstagramAPI_Response_ArchiveMediaResponse());
    }
    ...
    public function backup($baseOutputPath = null, $printProgress = true) {
        ...
        if ($baseOutputPath === null) {
            $baseOutputPath = InstagramAPI_Constants::SRC_DIR . '/../backups/';
        }

        // Ensure that the whole directory path for the backup exists.
        $backupFolder = $baseOutputPath . $this->ig->username . '/' . date('Y-m-d') . '/';
        if (!InstagramAPI_Utils::createFolder($backupFolder)) {
            throw new RuntimeException(sprintf(
                    'The "%s" backup folder is not writable.', $backupFolder
            ));
        }
        ...
        do {
            ...
            foreach ($myTimeline->getItems() as $item) {
                ...
                if ($item->media_type == InstagramAPI_Response_Model\Item::ALBUM) {
                    ...
                    foreach ($item->getCarouselMedia() as $subItem) {
                        ++$subPosition;
                        if ($subItem->media_type == InstagramAPI_Response_Model\CarouselMedia::PHOTO) {
                    ...
                    if ($item->media_type == InstagramAPI_Response_Model\Item::PHOTO) {
