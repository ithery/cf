 ...
 */
class InstagramAPI_Request_Internal extends InstagramAPI_Request_RequestCollection {
     ...
     */
    public function uploadSinglePhoto($targetFeed, $photoFilename, InternalMetadata $internalMetadata = null, array $externalMetadata = []) {
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_UploadPhotoResponse
     */
    public function uploadPhotoData($targetFeed, InstagramAPI_Request_Metadata_Internal $internalMetadata) {
        // Make sure we disallow some feeds for this function.
        if ($targetFeed == InstagramAPI_Constants::FEED_DIRECT) {
            throw new InvalidArgumentException(sprintf('Bad target feed "%s".', $targetFeed));
        }
        ...
        if ($internalMetadata->getPhotoDetails() !== null) {
            ...
            try {
                $photoData = InstagramAPI_Utils::createVideoIcon($targetFeed, $internalMetadata->getVideoDetails()->getFilename());
            } catch (\Exception $e) {
                // Re-package as InternalException, but keep the stack trace.
                throw new InstagramAPI_Exception_InternalException($e->getMessage(), 0, $e);
            }
        ...
        return $request->getResponse(new InstagramAPI_Response_UploadPhotoResponse());
    }
     ...
     *
     * @param int                                       $targetFeed       One of the FEED_X constants.
     * @param InstagramAPI_Request_Metadata_Internal    $internalMetadata Internal library-generated metadata object.
     * @param array                                     $externalMetadata (optional) User-provided metadata key-value pairs.
     *
     * @throws \InvalidArgumentException
     * @throws InstagramAPI_Exception_InstagramException
     ...
     * @return InstagramAPI_Response_ConfigureResponse
     */
    public function configureSinglePhoto($targetFeed, InstagramAPI_Request_Metadata_Internal $internalMetadata, array $externalMetadata = []) {
        // Determine the target endpoint for the photo.
        switch ($targetFeed) {
            case InstagramAPI_Constants::FEED_TIMELINE:
                $endpoint = 'media/configure/';
                break;
            case InstagramAPI_Constants::FEED_DIRECT_STORY:
            case InstagramAPI_Constants::FEED_STORY:
        ...
        /** @var InstagramAPI_Response_Model_Location|null A Location object describing where
        ...
        if ($location instanceof InstagramAPI_Response_Model_Location) {
            $request
                    ->addPost('location', InstagramAPI_Utils::buildMediaLocationJSON($location))
        ...
        $configure = $request->getResponse(new InstagramAPI_Response_ConfigureResponse());
     ...
     */
    public function uploadVideo($targetFeed, $videoFilename, InstagramAPI_Request_Metadata_Internal $internalMetadata = null) {
        if ($internalMetadata === null) {
            $internalMetadata = new InstagramAPI_Request_Metadata_Internal();
        }
     ...
     */
    public function uploadSingleVideo($targetFeed, $videoFilename, InstagramAPI_Request_Metadata_Internal $internalMetadata = null, array $externalMetadata = []) {
        // Make sure we only allow these particular feeds for this function.
        if ($targetFeed != InstagramAPI_Constants::FEED_TIMELINE && $targetFeed != InstagramAPI_Constants::FEED_STORY && $targetFeed != InstagramAPI_Constants::FEED_DIRECT_STORY) {
            throw new InvalidArgumentException(sprintf('Bad target feed "%s".', $targetFeed));
        }
        ...
        /** @var InstagramAPI_Response_ConfigureResponse $configure */
        $configure = $this->ig->internal->configureWithRetries(
                $videoFilename, function () use ($targetFeed, $internalMetadata, $externalMetadata) {
     ...
     * @param InternalMetadata $internalMetadata Internal library-generated metadata object.
     *
     * @throws InstagramAPI_Exception_InstagramException If the request fails.
     *
     * @return InstagramAPI_Response_UploadJobVideoResponse
     */
    protected function _requestVideoUploadURL($targetFeed, InstagramAPI_Request_Metadata_Internal $internalMetadata) {
        ...
        /** @var InstagramAPI_Response_UploadJobVideoResponse $response */
        $response = $request->getResponse(new InstagramAPI_Response_UploadJobVideoResponse());
     ...
     * @return InstagramAPI_Response_ConfigureResponse
     */
    public function configureSingleVideo($targetFeed, InstagramAPI_Request_Metadata_Internal $internalMetadata, array $externalMetadata = []) {
        ...
        $usertags = (isset($externalMetadata['usertags']) && $targetFeed == InstagramAPI_Constants::FEED_STORY) ? $externalMetadata['usertags'] : null;
        /** @var InstagramAPI_Response_Model_Location|null A Location object describing where
         * the media was taken. NOT USED FOR STORY MEDIA! */
        $location = (isset($externalMetadata['location']) && $targetFeed != InstagramAPI_Constants::FEED_STORY) ? $externalMetadata['location'] : null;
        /** @var string|null Link to attach to the media. ONLY USED FOR STORY MEDIA,
         * AND YOU MUST HAVE A BUSINESS INSTAGRAM ACCOUNT TO POST A STORY LINK! */
        $link = (isset($externalMetadata['link']) && $targetFeed == InstagramAPI_Constants::FEED_STORY) ? $externalMetadata['link'] : null;
        /** @var array Hashtags to use for the media. ONLY STORY MEDIA! */
        $hashtags = (isset($externalMetadata['hashtags']) && $targetFeed == InstagramAPI_Constants::FEED_STORY) ? $externalMetadata['hashtags'] : null;
        ...
        switch ($targetFeed) {
            ...
            case Constants::FEED_STORY:
                ...
                if (is_string($link) && InstagramAPI_Utils::hasValidWebURLSyntax($link)) {
                    $story_cta = '[{"links":[{"webUri":' . json_encode($link) . '}]}]';
                    $request->addPost('story_cta', $story_cta);
                }
                if ($hashtags !== null && $captionText != '') {
                    InstagramAPI_Utils::throwIfInvalidStoryHashtags($captionText, $hashtags);
                    $request
                            ->addPost('story_hashtags', json_encode($hashtags))
                            ->addPost('caption', $captionText)
                            ->addPost('mas_opt_in', 'NOT_PROMPTED');
        ...
        if ($targetFeed == InstagramAPI_Constants::FEED_STORY) {
    ...
    array $media, InternalMetadata $internalMetadata, array $externalMetadata = []) {
     ...
     *
     * @param InstagramAPI_Response_SyncResponse $syncResponse
     *
     * @throws InstagramAPI_Exception_SettingsException
     */
    protected function _saveExperiments(
    InstagramAPI_Response_SyncResponse $syncResponse) {
        $experiments = [];
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_SyncResponse
     */
    public function syncDeviceFeatures(
    $prelogin = false) {
        $request = $this->ig->request('qe/sync/')
                ->addPost('id', $this->ig->uuid)
                ->addPost('experiments', InstagramAPI_Constants::LOGIN_EXPERIMENTS);
        if ($prelogin) {
        ...
        return $request->getResponse(new InstagramAPI_Response_SyncResponse());
    }
    ...
    public function syncUserFeatures() {
        $result = $this->ig->request('qe/sync/')
                ...
                ->addPost('experiments', InstagramAPI_Constants::EXPERIMENTS)
                ->getResponse(new InstagramAPI_Response_SyncResponse());
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_MsisdnHeaderResponse
     *
    ...
    $subnoKey = null) {
        ...
        return $request->getResponse(new InstagramAPI_Response_MsisdnHeaderResponse());
    }
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_MegaphoneLogResponse
     */
    public function getMegaphoneLog() {
        return $this->ig->request('megaphone/log/')
                        ...
                        ->getResponse(new InstagramAPI_Response_MegaphoneLogResponse());
    }
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_FacebookHiddenEntitiesResponse
     */
    public function getFacebookHiddenSearchEntities() {
        return $this->ig->request('fbsearch/get_hidden_search_entities/')
                        ->getResponse(new InstagramAPI_Response_FacebookHiddenEntitiesResponse());
    }
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return InstagramAPI_Response_FacebookOTAResponse
     */
    public function getFacebookOTA() {
        return $this->ig->request('facebook_ota/')
                        ...
                        ->getResponse(new InstagramAPI_Response_FacebookOTAResponse());
    }
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return \InstagramAPI\InstagramAPI_Response_ProfileNoticeResponse
     */
    public function getProfileNotice() {
        return $this->ig->request('users/profile_notice/')
                        ...
                        ->addPost('_uid', $this->ig->account_id)
                        ->addPost('_csrftoken', $this->ig->client->getToken())
                        ->getResponse(new InstagramAPI_Response_ProfileNoticeResponse());
    }
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return \InstagramAPI\InstagramAPI_Response_FetchQPDataResponse
     */
    public function getQPFetch() {
        return $this->ig->request('qp/fetch/')
                        ...
                        ->getResponse(new InstagramAPI_Response_FetchQPDataResponse());
    }
     ...
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return \InstagramAPI\InstagramAPI_Response_ClientEventLogsResponse
     */
    ...
    array $data) {
        ...
        return $this->ig->request(Constants::GRAPH_URL . 'logging_client_events')
                        ...
                        ->addPost('format', 'json')
                        ->getResponse(new InstagramAPI_Response_ClientEventLogsResponse());
    }
     ...
     * @throws \InvalidArgumentException
     * @throws \RuntimeException
     * @throws InstagramAPI_Exception_UploadFailedException
     * @throws InstagramAPI_Exception_InstagramException
     *
     * @return ResponseInterface
     */
    public function configureWithRetries(
    $entity, callable $configurator) {
        $attempt = 0;
        while (true) {
            // Check for max retry-limit, and throw if we exceeded it.
            if (++$attempt > self::MAX_CONFIGURE_RETRIES) {
                throw new InstagramAPI_Exception_UploadFailedException(sprintf(
                        'Configuration of "%s" failed. All retries have failed.', $entity
                ));
            }
            ...
            switch ($httpResponse->getStatusCode()) {
                case 200:
                    ...
                    if (strtolower($result->getMessage()) === 'media_needs_reupload') {
                        throw new InstagramAPI_Exception_UploadFailedException(sprintf(
                                'Configuration of "%s" failed. You need to reupload the media (%s).', $entity, (isset($fullResponse->error_title) ? $fullResponse->error_title : 'unknown error')
                        ));
                    } elseif ($result->isOk()) {
                        return $result;
                    }
        ...
        throw new InstagramAPI_Exception_UploadFailedException(sprintf(
                'Configuration of "%s" failed.', $entity
        ));
    }
     ...
     * @throws \InvalidArgumentException
     * @throws InstagramAPI_Exception_InstagramException
     * @throws InstagramAPI_Exception_UploadFailedException If the upload fails.
     *
     * @return \InstagramAPI\InstagramAPI_Response_UploadVideoResponse
     */
    protected function _uploadVideoChunks(
    $targetFeed, InternalMetadata $internalMetadata) {
        ...
        if ($targetFeed == Constants::FEED_TIMELINE_ALBUM) {
            ...
            if ($sessionIDCookie === null) { // Verify value.
                throw new InstagramAPI_Exception_UploadFailedException(
        ...
        if (!is_array($uploadUrls) || !count($uploadUrls)) {
            throw new InstagramAPI_Exception_UploadFailedException('No video upload URLs found.');
        }
        ...
        if ($handle === false) {
            throw new InstagramAPI_Exception_UploadFailedException(sprintf(
                    'Failed to open "%s" for reading.', $videoFilename
            ));
        }
        ...
        try {
            ...
            while (true) {
                ...
                if ($uploadUrl === null) {
                    ...
                    if ($uploadUrl === null) {
                        throw new InstagramAPI_Exception_UploadFailedException(sprintf(
                                'Upload of "%s" failed. There are no more upload URLs.', $videoFilename
                        ));
                    }
                    // Reset state.
                ...
                try {
                    /** @var InstagramAPI_Response_UploadVideoResponse $result */
                    $result = $request->getResponse(new InstagramAPI_Response_UploadVideoResponse());
                } catch (CheckpointRequiredException $e) {
                    throw $e;
                } catch (LoginRequiredException $e) {
                    throw $e;
                ...
                switch ($httpResponse->getStatusCode()) {
                    ...
                    case 511:
                        throw new InstagramAPI_Exception_UploadFailedException(sprintf(
                                "Upload of \"%s\" failed. Instagram's server returned HTTP status \"%d\".", $videoFilename, $httpResponse->getStatusCode()
                        ));
                    case 422:
                        throw new InstagramAPI_Exception_UploadFailedException(sprintf(
                                "Upload of \"%s\" failed. Instagram's server says that the video is corrupt.", $videoFilename, $httpResponse->getStatusCode()
                        ));
                    default:
                }
            }
        ...
        throw new InstagramAPI_Exception_UploadFailedException(sprintf(
                'Upload of \"%s\" failed.', $videoFilename
        ));
    }
     ...
     * @throws InstagramAPI_Exception_UploadFailedException If the upload fails.
     *
     * @return \InstagramAPI\InstagramAPI_Response_GenericResponse
     */
    ...
    $targetFeed, InternalMetadata $internalMetadata) {
        ...
        $endpoint = sprintf('https://i.instagram.com/rupload_igvideo/%s_%d_%d?target=%s', $internalMetadata->getUploadId(), 0, Utils::hashCode($videoFilename), $rurCookie->getValue()
        );
        ...
        try {
            ...
            while (true) {
                ...
                if (++$attempt > self::MAX_RESUMABLE_RETRIES) {
                    throw new InstagramAPI_Exception_UploadFailedException(sprintf(
                            'Upload of "%s" failed. All retries have failed.', $videoFilename
                    ));
                }
                ...
                try {
                    ...
                    /** @var InstagramAPI_Response_ResumableOffsetResponse $offsetResponse */
                    $offsetResponse = $offsetRequest->getResponse(new InstagramAPI_Response_ResumableOffsetResponse());
                    ...
                    /** @var InstagramAPI_Response_GenericResponse $response */
                    $response = $uploadRequest->getResponse(new InstagramAPI_Response_GenericResponse());
        ...
        switch ($targetFeed) {
            ...
            case Constants::FEED_TIMELINE:
                $result = $this->ig->isExperimentEnabled(
                        'ig_android_upload_reliability_universe', 'is_enabled_fbupload_followers_share');
                break;
            case Constants::FEED_DIRECT:
                $result = $this->ig->isExperimentEnabled(
                        'ig_android_upload_reliability_universe', 'is_enabled_fbupload_direct_share');
                break;
            case Constants::FEED_STORY:
                $result = $this->ig->isExperimentEnabled(
                        'ig_android_upload_reliability_universe', 'is_enabled_fbupload_reel_share');
                break;
            case Constants::FEED_DIRECT_STORY:
                $result = $this->ig->isExperimentEnabled(
                        'ig_android_upload_reliability_universe', 'is_enabled_fbupload_story_share');
                break;
            default:
                $result = false;
        }
    ...
    $targetFeed, InternalMetadata $internalMetadata) {
        ...
        $result = [
            ...
            'media_type' => (string) InstagramAPI_Response_Model_Item::VIDEO,
        ];
