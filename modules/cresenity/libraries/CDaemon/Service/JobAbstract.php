...
abstract class CDaemon_Service_JobAbstract extends CDaemon_ServiceAbstract implements CDaemon_Service_JobInterface {
     ...
     * is literally no difference between using this and just passing the message to static::log().
     *
     * @param $message
     * @param string $label
     */
    ...
    private function timer($start = false) {
        ...
        if ($is_idle()) {
            $this->dispatch(array(static::ON_IDLE), array($is_idle));
        }
        $stats = array();
    ...
    /**
     * Create a persistent Worker process. This is an object loader similar to static::plugin().
     *
