<?php
    ...
    class CDatabase {
        ...
        public function __construct($config = array(), $domain = null) {
            ...
            // Set driver name
            $driver = 'CDatabase_' . ucfirst($this->config['connection']['type']) . '_Driver';

            // Load the driver
            $driver_file = dirname(__FILE__) . "/drivers/CDatabase/" . ucfirst($this->config['connection']['type']) . EXT;

            if (!class_exists('CDatabase_Driver')) {
                require_once dirname(__FILE__) . "/drivers/CDatabase" . EXT;
            }

            if (!file_exists($driver_file)) {
                throw new CDatabase_Exception('core.driver_not_found', $this->config['connection']['type'], get_class($this));
            }
            else {
            ...
            if (!($this->driver instanceof CDatabase_Driver))
                    throw new CDatabase_Exception('core.driver_implements', $this->config['connection']['type'], get_class($this), 'CDatabase_Driver');

            CF::log('debug', 'Database Library initialized');
        }
        ...
        public function connect() {
            ...
            if (!is_resource($this->link) AND ! is_object($this->link)) {
                ...
                if (!is_resource($this->link) AND ! is_object($this->link))
                        throw new CDatabase_Exception('database.connection', $this->driver->show_error());
        ...
        public function set($key, $value = '') {
            ...
            foreach ($key as $k => $v) {
                ...
                if (strpos($k, '.')) $k = $this->config['table_prefix'] . $k;
        ...
        public function table_exists($table_name, $prefix = TRUE) {
            ...
            else return in_array($table_name, $this->list_tables());
        }
        ...
        public function compile_binds($sql, $binds) {
            foreach ((array) $binds as $val) {
                ...
                if (($next_bind_pos = strpos($sql, '?')) === FALSE) break;
        ...
        public function begin() {
            if (!$this->in_trans) $this->query('START TRANSACTION;');
            $this->in_trans = true;
        }

        public function commit() {
            if ($this->in_trans) $this->query('COMMIT;');
            $this->in_trans = false;
        }

        public function rollback() {
            if ($this->in_trans) $this->query('ROLLBACK;');
            $this->in_trans = false;
        }
...
// End Database Class
     ...
     */
    class CDatabase_Exception extends CF_Exception {
