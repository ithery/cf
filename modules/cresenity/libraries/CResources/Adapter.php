 ...
 */
class CResources_Adapter implements CResources_Interface_FilesystemInterface, CResources_Interface_CloudFilesystemInterface {
    ...
    public function getRackspaceTemporaryUrl($adapter, $path, $expiration, $options) {
        return $adapter->getContainer()->getObject($path)->getTemporaryUrl(
                        Carbon::now()->diffInSeconds($expiration), isset($options['method']) ? $options['method'] : 'GET', isset($options['forcePublicUrl']) ? $options['forcePublicUrl'] : true
        );
    }
