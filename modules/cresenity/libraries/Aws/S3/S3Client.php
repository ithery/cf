 ...
 */
class Aws_S3_S3Client extends Aws_AwsClient implements Aws_S3_S3ClientInterface {

    use Aws_S3_S3ClientTrait;
    ...
    public function __construct(array $args) {
        ...
        $stack->appendInit(Aws_S3_SSECMiddleware::wrap($this->getEndpoint()->getScheme()), 's3.ssec');
        $stack->appendBuild(Aws_S3_ApplyChecksumMiddleware::wrap(), 's3.checksum');
        $stack->appendBuild(
                Aws_Middleware::contentType(['PutObject', 'UploadPart']), 's3.content_type'
        );
        ...
        if ($this->getConfig('bucket_endpoint')) {
            ...
            $stack->appendBuild(
                    Aws_S3_S3EndpointMiddleware::wrap(
                            $this->getRegion(), [
        ...
        $stack->appendSign(Aws_S3_PutObjectUrlMiddleware::wrap(), 's3.put_object_url');
        $stack->appendSign(Aws_S3_PermanentRedirectMiddleware::wrap(), 's3.permanent_redirect');
        $stack->appendInit(Aws_Middleware::sourceFile($this->getApi()), 's3.source_file');
    ...
    /** @internal */
    public static function _applyRetryConfig($value, $_, Aws_HandlerList $list) {
        if (!$value) {
            return;
        }

        $decider = Aws_RetryMiddleware::createDefaultDecider($value);
        ...
        $delay = [Aws_RetryMiddleware::class, 'exponentialDelay'];
        $list->appendSign(Aws_Middleware::retry($decider, $delay), 'retry');
    }

    /** @internal */
    public static function _applyApiProvider($value, array &$args, Aws_HandlerList $list) {
        Aws_ClientResolver::_apply_api_provider($value, $args, $list);
        $args['parser'] = new Aws_S3_GetBucketLocationParser(
                new Aws_S3_AmbiguousSuccessParser(
                new Aws_S3_RetryableMalformedResponseParser(
