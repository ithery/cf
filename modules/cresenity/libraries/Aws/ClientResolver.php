...
class Aws_ClientResolver {
    ...
    private static $defaultArgs = [
        ...
        'exception_class' => [
            ...
            'default' => 'Aws_Exception_AwsException',
            'internal' => true
        ],
        'scheme' => [
            'type' => 'value',
        ...
        'api_provider' => [
            ...
            'default' => [Aws_Api_ApiProvider::class, 'defaultProvider'],
        ],
        'endpoint_provider' => [
            'type' => 'value',
            'valid' => ['callable'],
        ...
        'credentials' => [
            'type' => 'value',
            'valid' => [Aws_Credentials_CredentialsInterface::class, CacheInterface::class, 'array', 'bool', 'callable'],
            'doc' => 'Specifies the credentials used to sign requests. Provide an Aws\Credentials\CredentialsInterface object, an associative array of "key", "secret", and an optional "token" key, `false` to use null credentials, or a callable credentials provider used to create credentials or return null. See Aws\\Credentials\\CredentialProvider for a list of built-in credentials providers. If no credentials are provided, the SDK will attempt to load them from the environment.',
            'fn' => [__CLASS__, '_apply_credentials'],
            'default' => [Aws_Credentials_CredentialProvider::class, 'defaultProvider'],
        ],
        'stats' => [
            'type' => 'value',
            'valid' => ['bool', 'array'],
     ...
     */
    public function resolve(array $args, Aws_HandlerList $list) {
    ...
    public static function _apply_credentials($value, array &$args) {
        if (is_callable($value)) {
            ...
            $args['credentials'] = Aws_Credentials_CredentialProvider::fromCredentials(
                            new Aws_Credentials_Credentials(
    ...
    public static function _apply_api_provider(callable $value, array &$args) {
        $api = new Aws_Api_Service(
                Aws_Api_ApiProvider::resolve(
        ...
        $args['api'] = $api;
        $args['parser'] = Aws_Api_Service::createParser($api);
        $args['error_parser'] = Aws_Api_Service::createErrorParser($api->getProtocol());
    }
    ...
    public static function _apply_endpoint_provider(callable $value, array &$args) {
        if (!isset($args['endpoint'])) {
            ...
            $result = Aws_Endpoint_EndpointProvider::resolve($value, [
    ...
    public static function _apply_serializer($value, array &$args, Aws_HandlerList $list) {
    ...
    public static function _apply_stats($value, array &$args, Aws_HandlerList $list) {
    ...
    public static function _apply_validate($value, array &$args, Aws_HandlerList $list) {
        if ($value === false) {
            return;
        }

        $validator = $value === true ? new Aws_Api_Validator() : new Aws_Api_Validator($value);
        $list->appendValidate(
                Aws_Middleware::validation($args['api'], $validator), 'validation'
        );
    }

    public static function _apply_handler($value, array &$args, Aws_HandlerList $list) {
        $list->setHandler($value);
    }

    public static function _default_handler(array &$args) {
        return new Aws_WrappedHttpHandler(
                aws_default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']
        );
    }
    ...
    public static function _apply_user_agent($value, array &$args, Aws_HandlerList $list) {
        if (!is_array($value)) {
            $value = [$value];
        }
        ...
        array_unshift($value, 'aws-sdk-php/' . Aws_Sdk::VERSION);
    ...
    public static function _apply_endpoint($value, array &$args, Aws_HandlerList $list) {
    ...
    $value, array &$args, Aws_HandlerList $list
    ) {
        $enabled = false;
        $generator = null;
        ...
        if ($enabled) {
            $list->prependInit(
                    Aws_IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill'
            );
        }
    }

    public static function _default_endpoint_provider(array $args) {
        return Aws_Endpoint_PartitionEndpointProvider::defaultProvider()
                        ->getPartition($args['region'], $args['service']);
    }

    public static function _default_serializer(array $args) {
        return Aws_Api_Service::createSerializer(
                        $args['api'], $args['endpoint']
        );
    }

    public static function _default_signature_provider() {
        return Aws_Signature_SignatureProvider::defaultProvider();
    }
    ...
    public static function _default_signature_version(array &$args) {
        ...
        $args['__partition_result'] = isset($args['__partition_result']) ? isset($args['__partition_result']) : call_user_func(Aws_Endpoint_PartitionEndpointProvider::defaultProvider(), [
