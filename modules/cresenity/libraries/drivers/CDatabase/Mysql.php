...
class CDatabase_Mysql_Driver extends CDatabase_Driver {
	...
	public function __construct($config)
	{
		...
		CF::log('debug', 'MySQL Database Driver Initialized');
	}
	...
	public function query($sql)
	{
		...
		if ($this->db_config['cache'] AND ! preg_match('#\b(?:INSERT|UPDATE|REPLACE|SET|DELETE|TRUNCATE)\b#i', $sql))
		{
			...
			if ( ! isset($this->query_cache[$hash]))
			{
				...
				$this->query_cache[$hash] = new CMysql_Result(mysql_query($sql, $this->link), $this->link, $this->db_config['object'], $sql);
			}
			else
			{
		...
		return new CMysql_Result(mysql_query($sql, $this->link), $this->link, $this->db_config['object'], $sql);
	}
	...
	public function list_fields($table)
	{
		...
		if (!isset($result))
			throw new CDatabase_Exception('database.table_not_found', $table);
 ...
 * MySQL Result
 */
class CMysql_Result extends CDatabase_Result {
	...
	public function __construct($result, $link, $object = TRUE, $sql)
	{
		...
		elseif (is_bool($result))
		{
			...
			{
				// SQL error
				throw new CDatabase_Exception('database.error', mysql_error($link).' - '.$sql);
			}
			else
			{
	...
	public function result_array($object = NULL, $type = MYSQL_ASSOC)
	{
		...
		else
		{
			...
			{
				$type = (is_string($this->return_type) AND CF::auto_load($this->return_type)) ? $this->return_type : 'stdClass';
			}
		}
