...
class Elasticsearch_Client {
     ...
     */
    public function __construct(Elasticsearch_Transport $transport, callable $endpoint, array $registeredNamespaces) {
        $this->transport = $transport;
        $this->endpoints = $endpoint;
        $this->indices = new Elasticsearch_Namespaces_IndicesNamespace($transport, $endpoint);
        $this->cluster = new Elasticsearch_Namespaces_ClusterNamespace($transport, $endpoint);
        $this->nodes = new Elasticsearch_Namespaces_NodesNamespace($transport, $endpoint);
        $this->snapshot = new Elasticsearch_Namespaces_SnapshotNamespace($transport, $endpoint);
        $this->cat = new Elasticsearch_Namespaces_CatNamespace($transport, $endpoint);
        $this->ingest = new Elasticsearch_Namespaces_IngestNamespace($transport, $endpoint);
        $this->tasks = new Elasticsearch_Namespaces_TasksNamespace($transport, $endpoint);
        $this->remote = new Elasticsearch_Namespaces_RemoteNamespace($transport, $endpoint);
    ...
    public function ping($params = []) {
        ...
        } catch (Elasticsearch_Common_Exceptions_Missing404Exception $exception) {
            return false;
        } catch (Elasticsearch_Common_Exceptions_TransportException $exception) {
            return false;
        } catch (Elasticsearch_Common_Exceptions_NoNodesAvailableException $exception) {
            return false;
        }
     ...
     * @return array
     */
    private function performRequest(Elasticsearch_Endpoints_AbstractEndpoint $endpoint) {
