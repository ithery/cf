 ...
 */
class Elasticsearch_ClientBuilder {
    ...
    /** @var string */
    private $connectionPool = 'Elasticsearch_ConnectionPool_StaticNoPingConnectionPool';

    /** @var  string */
    private $serializer = 'Elasticsearch_Serializers_SmartSerializer';

    /** @var  string */
    private $selector = 'Elasticsearch_ConnectionPool_Selectors_RoundRobinSelector';
    ...
    public static function defaultHandler($multiParams = [], $singleParams = []) {
        ...
        if (extension_loaded('curl')) {
            ...
            if (function_exists('curl_reset')) {
                $default = new GuzzleHttp_Ring_Client_CurlHandler($singleParams);
                $future = new GuzzleHttp_Ring_Client_CurlMultiHandler($config);
            } else {
                $default = new GuzzleHttp_Ring_Client_CurlMultiHandler($config);
            }
        } else {
            throw new \RuntimeException('Elasticsearch-PHP requires cURL, or a custom HTTP handler.');
        }

        return $future ? GuzzleHttp_Ring_Client_Middleware::wrapFuture($default, $future) : $default;
    }
    ...
    public function setSerializer($serializer) {
        $this->parseStringOrObject($serializer, $this->serializer, 'Elastisearch_Serializers_SerializerInterface');
    ...
    public function build() {
        ...
        if (is_null($this->handler)) {
            $this->handler = Elasticsearch_ClientBuilder::defaultHandler();
        }
        ...
        if (is_null($this->serializer)) {
            $this->serializer = new Elasticsearch_Serializers_SmartSerializer();
        ...
        if (is_null($this->connectionFactory)) {
            ...
            $this->connectionFactory = new Elasticsearch_Connections_ConnectionFactory($this->handler, $this->connectionParams, $this->serializer, $this->logger, $this->tracer);
        }
        ...
        if (is_null($this->endpoint)) {
            ...
            $this->endpoint = function ($class) use ($serializer) {
                $fullPath = 'Elasticsearch_Endpoints_' . $class;
     ...
     * @return Client
     */
    protected function instantiate(Elasticsearch_Transport $transport, callable $endpoint, array $registeredNamespaces) {
        return new Elasticsearch_Client($transport, $endpoint, $registeredNamespaces);
    }

    private function buildLoggers() {
        if (is_null($this->logger)) {
            $this->logger = new Psr_Log_NullLogger();
        }

        if (is_null($this->tracer)) {
            $this->tracer = new Psr_Log_NullLogger();
        }
    }
    ...
    private function buildTransport() {
        ...
        if (is_string($this->connectionPool)) {
            $this->connectionPool = new $this->connectionPool(
                    $connections, $this->selector, $this->connectionFactory, $this->connectionPoolArgs);
        ...
        if (is_null($this->transport)) {
            $this->transport = new Elasticsearch_Transport($this->retries, $this->sniffOnStart, $this->connectionPool, $this->logger);
        }
    }
