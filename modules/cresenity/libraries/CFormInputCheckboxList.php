	...
	public function html($indent=0) {
			...
			foreach($this->group_list as $g=>$list) {
				...
				foreach($list as $k=>$v) {
					...
					if(is_array($this->value)) {
						if(in_array($k,$this->value)) $selected=' selected="selected"';
					} else {
						if($this->value==(string)$k) $selected=' selected="selected"';
					}
		...
		if($this->list!=null) {
			...
			foreach($this->list as $k=>$v) {
				...
				$checked = "";
				if(is_array($this->value)) {
					if(in_array($k,$this->value)) $checked=' checked="checked"';
				} else {
					if($this->value==(string)$k) $checked=' checked="checked"';
				}
