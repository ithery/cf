 ...
 */
class CElastic_Client_Query extends CElastic_Param {
     ...
     * @param mixed $query
     *
     * @throws CElastic_Exception_InvalidException For an invalid argument
     *
     * @return self
     */
    public static function create($query) {
        switch (true) {
            case $query instanceof self:
                return $query;
            case $query instanceof CElastic_Client_Query_AbstractQuery:
                return new self($query);
            case empty($query):
                return new self(new CElastic_Client_Query_MatchAll());
            case is_array($query):
                return new self($query);
            case is_string($query):
                return new self(new CElastic_Client_Query_QueryString($query));
            case $query instanceof CElastic_Client_Suggest_AbstractSuggest:
                return new self(new CElastic_Client_Suggest($query));
            case $query instanceof CElastic_Client_Suggest:
                return new self($query);
        }
        throw new CElastic_Exception_InvalidException('Unexpected argument to create a query for.');
    }
     ...
     * Sets the query.
     *
     * @param CElastic_Client_Query_AbstractQuery $query Query object
     *
     * @return $this
     */
    public function setQuery(CElastic_Client_Query_AbstractQuery $query) {
    ...
    public function toArray() {
        ...
        if (!isset($this->_params['query']) && ($this->_suggest == 0)) {
            $this->setQuery(new CElastic_Client_Query_MatchAll());
        }
         * 
         */
        $paramPostFilter = carr::get($this->_params, 'post_filter');
        if ($paramPostFilter != null && count($paramPostFilter->toArray()) === 0) {
