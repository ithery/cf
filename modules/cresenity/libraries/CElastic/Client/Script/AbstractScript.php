...
abstract class CElastic_Client_Script_AbstractScript extends CElastic_Client_AbstractUpdateAction {
     ...
     * @param null|string $lang       Script language, see constants
     * @param null|string $documentId Document ID the script action should be performed on (only relevant in update context)
     */
    ...
    private static function _createFromArray(array $data) {
        $params = carr::get($data, 'script.params', []);
        $lang = carr::get($data, 'script.lang', null);
        if (!is_array($params)) {
