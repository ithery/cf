...
class CElastic_DSL_SearchBuilder {
     ...
     * @param Query      $grammar
     */
    public function __construct(CElastic $connection, Query $grammar = null) {
     ...
     * @return $this
     */
    public function model(CElastic_Model $model) {
        // Check if the model is searchable before setting the query builder model
        $traits = CF::class_uses_recursive(get_class($model));

        if (!isset($traits[CElastic_Trait_Searchable::class])) {
    ...
    public function getModelFiller() {
        return $this->modelFiller ? $this->modelFiller : new CElastic_Model_Filler();
    }
    ...
    public function get() {
        ...
        $result = new CElastic_Result($result);
