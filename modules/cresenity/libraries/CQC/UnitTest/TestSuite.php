 ...
 */
class CQC_UnitTest_TestSuite implements IteratorAggregate, CQC_UnitTest_ReorderableInterface, CQC_SelfDescribing, Test {
    ...
    public function __construct($theClass = '', string $name = '') {
        if (!is_string($theClass) && !$theClass instanceof ReflectionClass) {
            throw InvalidArgumentException::create(
                    1, 'ReflectionClass object or string'
            );
        }
        ...
        try {
            ...
            foreach ($this->tests() as $test) {
                ...
                if (!$errorAdded) {
                    ...
                    $result->addFailure(
                            $test, new SkippedTestError('Test skipped because of an error in hook method'), 0
                    );
                }
    ...
    public function getIterator() {
        $iterator = new CQC_UnitTest_TestSuiteIterator($this);
        ...
        if ($this->requiredTests === null) {
            ...
            foreach ($this->tests as $test) {
                ...
                $this->requiredTests = ExecutionOrderDependency::mergeUnique(
                                ExecutionOrderDependency::filterInvalid($this->requiredTests), $test->requires()
                );
            }
    ...
    protected function addTestMethod(ReflectionClass $class, ReflectionMethod $method) {
        ...
        $this->addTest(
                $test, TestUtil::getGroups($class->getName(), $methodName)
        );
    }
