 ...
 *
 * @since Jun 19, 2018, 5:09:43 AM
 */
class CGitlab_Api_Repositories extends CGitlab_Api {
    /**
     * @param int $project_id
     *
     * @return mixed
     */
    public function branches($project_id, array $params = []) {
    ...
    public function createRelease($project_id, $tag_name, $description) {
        return $this->post($this->getProjectPath($project_id, 'repository/tags/' . $this->encodeBranch($tag_name) . '/release'), [
            'id' => $project_id,
            'tag_name' => $tag_name,
    ...
    public function updateRelease($project_id, $tag_name, $description) {
        return $this->put($this->getProjectPath($project_id, 'repository/tags/' . $this->encodeBranch($tag_name) . '/release'), [
            'id' => $project_id,
            'tag_name' => $tag_name,
            'description' => $description
        ]);
    ...
    public function commitBuilds($project_id, $sha, $scope = null, $page = 0, $per_page = self::PER_PAGE) {
        return $this->get($this->getProjectPath($project_id, 'repository/commits/' . $this->encodePath($sha) . '/builds'), [
            'page' => $page,
            'per_page' => $per_page,
            'scope' => $scope
        ]);
    ...
    public function commitComments($project_id, $sha, $page = 0, $per_page = self::PER_PAGE) {
        return $this->get($this->getProjectPath($project_id, 'repository/commits/' . $this->encodePath($sha) . '/comments'), [
            'page' => $page,
            'per_page' => $per_page
        ]);
    }
    ...
    public function blob($project_id, $sha, $filepath) {
        return $this->get($this->getProjectPath($project_id, 'repository/commits/' . $this->encodePath($sha) . '/blob'), [
            'filepath' => $filepath
        ]);
    }
    ...
    public function createFile($project_id, $file_path, $content, $branch_name, $commit_message, $encoding = null) {
        return $this->post($this->getProjectPath($project_id, 'repository/files/' . $this->encodePath($file_path)), [
            'branch' => $branch_name,
            'content' => $content,
    ...
    public function updateFile($project_id, $file_path, $content, $branch_name, $commit_message, $encoding = null) {
        return $this->put($this->getProjectPath($project_id, 'repository/files/' . $this->encodePath($file_path)), [
            'branch' => $branch_name,
            'content' => $content,
            'commit_message' => $commit_message,
            'encoding' => $encoding
    ...
    public function deleteFile($project_id, $file_path, $branch_name, $commit_message) {
        return $this->delete($this->getProjectPath($project_id, 'repository/files' . $this->encodePath($file_path)), [
            'branch' => $branch_name,
            'commit_message' => $commit_message
        ]);
    }
     ...
     * @return mixed
     */
    public function archive($project_id, $params = []) {
